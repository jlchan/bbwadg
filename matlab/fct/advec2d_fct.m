...
global Qr Qs Qrskew Qsskew wJq Fmask E 
global QrFV QsFV QrskewFV QsskewFV
...
QrskewFV = .5*(QrFV-QrFV');
QsskewFV = .5*(QsFV-QsFV');
...
% flux = .5*nx.*(uP);
rhsL = .5*E*(sJ.*flux);
rhsH = rxJ.*(Qr*u) + sxJ.*(Qs*u) + E*(sJ.*flux);

% upwinding 
dflux = .5*abs(nx).*(uP-uM);
rhsH = rhsH - E*(sJ.*dflux);
rhsL = rhsL - E*(sJ.*dflux);
...
for e = 1:size(u,2)        
    ...
    Lmax = abs(QxFV./(D2+1e-10)); % | beta dot nij |
...
%     uL = u(:,e) + dt*sum(FL,2)./wJq(:,e);
%     uL = repmat(uL(:),1,size(u,1)); 
%     lj = 1/size(u,1);
%     P = diag(dt./wJq(:,e))*(A./lj);
        
    % uL + L.*P > 0 => L > -uL./P
    L = 1*ones(size(A));
    % ids = uL < 0;
    %     L(ids) = -uL(ids)./(P(ids) + 1e-10);
    %     L = max(0,min(1,L));
    %     L = rand(size(A));
    %     L = .5*(L + L');
    keyboard
