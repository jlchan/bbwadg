...
% project BC data onto finer mesh data and downsample in time
NBfaces = length(mapB)/Nfp;
D = [eye(Ncoarse+1) zeros(Ncoarse+1,N-Ncoarse)];
F = Vandermonde1D(Ncoarse,JacobiGL(0,0,Ncoarse))*(D/Vandermonde1D(N,JacobiGL(0,0,N)));
Ubc_coarse1 = zeros(NBfaces*(Ncoarse+1),NstepRK);
Ubc_coarse2 = zeros(NBfaces*(Ncoarse+1),NstepRK);
for i = 1:NstepRK
    Ubc1 = reshape(Ubc{1}(:,i),Nfp,NBfaces);
    Ubc2 = reshape(Ubc{2}(:,i),Nfp,NBfaces);
    F1 = F*Ubc1;
    F2 = F*Ubc2;
    Ubc_coarse1(:,i) = F1(:) + .01*norm(F1(:))*randn(size(F1(:)));
    Ubc_coarse2(:,i) = F2(:) + .01*norm(F2(:))*randn(size(F2(:)));
end
Ubc{1} = Ubc_coarse1;
Ubc{2} = Ubc_coarse2;
