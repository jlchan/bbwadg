...
if ~nargin || ~ischar( varargin{1} )
    ...
    switch action
        ...
        otherwise
            iDoBadInputErrorDialog( varargin{1} );
    end % switch action
end
end  % splinetool
...
switch ip
    ...
    case 2
        if length(onoff)==3
            set(hand.tool(2),'UserData',get(legend(hand.Axes(1)),'Position'));
...
if names(V,1)=='v'
    set_legend(hand,hand.Axes(1),get(legend(hand.Axes(1)),'Position'))
end
end  % iDoChangeName
...
function iDoBadInputErrorDialog(x)
errordlg( ...
    getString(message('SPLINES:resources:dlg_StringOneInput', x )), ...
