...
for i = 1:maxDegree
    for j = i:-1:0
        if j <= m && (i-j) <=n
            ...
            % The derivative of this wrt to x is j * x^(j-1) * y^(i-j)
            fx = fx + coeffs(k) * j * x.^(j-1) .* y.^(i-j);
            
            % The derivative of this wrt to y is (i-j) * x^(j) * y^(i-j-1)
            fy = fy + coeffs(k) * (i-j) * x.^(j) .* y.^(i-j-1);
   
            if doSecondDerivatives
                % The 2nd derivative of this wrt to x is (j-1)*j * x^(j-2) * y^(i-j)
                fxx = fxx + coeffs(k) * j * (j-1) * x.^(j-2) .* y.^(i-j);
                
                % The 2nd derivative of this wrt to y is (i-j) *(i-j-1) * x^(j) * y^(i-j-2)
                fyy = fyy + coeffs(k) * (i-j) * (i-j-1) * x.^(j) .* y.^(i-j-2);
                
                % The mixed 2nd derivative of this is j * (i-j) * x^(j-1) * y^(i-j-1)
                fxy = fxy + coeffs(k) * j * (i-j) * x.^(j-1) .* y.^(i-j-1);
            end
        end
    end
end
end  % polySurfaceDerivative
