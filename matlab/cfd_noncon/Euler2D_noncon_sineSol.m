...
Lx = max(x(:))-min(x(:));
Ly = max(y(:))-min(y(:));
x = x + a*Lx*cos(pi/Lx.*(x-0*Lx/2)).*cos(3*pi/Ly.*y);
y = y + a*Ly*sin(2*pi/Lx.*(x-0*Lx/2)).*cos(pi/Ly.*y);

% [r1 s1] = Nodes2D(1);
% I1N = Vandermonde2D(N,r1,s1)/Vandermonde2D(N,r,s);
% IN1 = Vandermonde2D(1,r,s)/Vandermonde2D(1,r1,s1);
% F = IN1*I1N;
% x = F*x;
% y = F*y;
