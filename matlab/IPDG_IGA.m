...
%     A = MM*A; % remove implicit mass matrix inverse
    ...
    for k = 1:Np*K
        wex = @(x) sin(k*pi*(1+x)/2);
        wexq = wex(xq); 
        wexnorm = sqrt(sum(sum(wJq.*(wexq.^2))));
                
        wDGq = Vq*reshape(W(:,k),Np,K);
        wDGnorm = wDGq(:)'*(wJq(:).*wexq(:)) / (wJq(:)'*wDGq(:).^2); % do L2 projection for scaling
        ...
        %         if k==1
%             plot(xp,wex(xp)/wexnorm,'--')
%             hold on
%             plot(xp,Vp*reshape(wDG,Np,K))
%                         
%             keyboard
%         end
    ...
    % plot(lamex,'o');hold on;plot(lam,'x')
    % semilogy(abs(lamex(:)-lam),'o')
%     hold on
    loglog(eigerr,'o--')
    hold on
...
% evaluate fluxes
hmin = 2.0/max(max(rx)); tau = (N+1)^2/hmin;
