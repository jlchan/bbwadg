function [nnzs conds] = Advec2D_precon(Nin)
...
%     title(sprintf('Largest real part = %e',max(real(lam))))
    ...
    A = eye(size(A)) - .1*A;
    Aavg = eye(size(A)) - .1*Aavg;

    P = {};
    Pavg = {};
    for e = 1:K
        ids = (1:Np) + (e-1)*Np;
        Pavg{e} = Aavg(ids,ids); % block Jacobi
        P{e} = A(ids,ids); % block Jacobi
    end
    P = blkdiag(P{:});    
    P(abs(P)<1e-6) = 0;
    Pavg = blkdiag(Pavg{:});    
    Pavg(abs(Pavg)<1e-6) = 0;
    
    %     nnz(P)/nnz(Pavg)
    nnzs = [(Np*Np*K) nnz(Pavg)];
    conds = [cond(T2*A*T1) cond(T2*(P\A)*T1) cond(T2*(Pavg\A)*T1)];
