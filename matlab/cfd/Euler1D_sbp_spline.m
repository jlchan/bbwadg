...
if useSplines > 0
    ...
    % re Initialize solver and construct grid and metric
    StartUp1D;
    ...
    [BVDM M Dr R rq wq Vq, ~, VXsub] = bsplineVDM(NB,Ksub,rB,smoothKnots,N); % VDM for interp, mass, M\S
    Nq = length(wq);
...
%         global xq2 VqPq2
        rhoq1 = VqPq2*rhoq([Nq+1,1:Nq,Nq+2],:);
        rhoq2 = [Vf(1,:);Vq;Vf(2,:)]*rho;
        mq1 = VqPq2*mq([Nq+1,1:Nq,Nq+2],:);
        mq2 = [Vf(1,:);Vq;Vf(2,:)]*m;
        Eq1 = VqPq2*Eq([Nq+1,1:Nq,Nq+2],:);
        Eq2 = [Vf(1,:);Vq;Vf(2,:)]*E;        
...
% %             semilogy(xq2,max(0,S1-S2),'bo','linewidth',2)
%             semilogy(xq2,abs(S1-S2),'b-','linewidth',2)
% %             semilogy(xq2,abs(rhoq1-rhoq2),'b-','linewidth',2)
% %             hold on
