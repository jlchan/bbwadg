...
if opt==1 % sineshock
    ...
    rhoL = 3.857143; rhoR = 1 + .2*sin(5*5);
    pL   = 10.3333;  pR = 1;
    uL   = 2.629369; uR = 0;
    
    rhoq = rhoL*(xq < -4) + (1 + .2*sin(5*xq)).*(xq >= -4);
    uq = uL*(xq < -4);
    pq = pL*(xq < -4) + pR*(xq >= -4);
    rho = PqG*(rhoq);
    u = PqG*(uq);
    p = PqG*(pq);
    E = PqG*(pq/(gamma-1) + .5*rhoq.*uq.^2);
    m = PqG*(rhoq.*uq);
...
F = kron(eye(K),V*diag([1;zeros(N,1)])/V);
...
for i = 1:Nsteps
    for INTRK = 1:5
        ...
        rhoP = tP*rhoq; uP = tP*uq; mP = tP*mq; EP = tP*Eq;
        rhoP(mapB) = [rhoL rhoR];
        uP(mapB)   = [uL uR];
        mP(mapB)   = uP(mapB).*rhoP(mapB);
        EP(mapB)   = [EL ER];
        ...
        d1 = PqG*(sum(Dh.*f2(rhox,rhoy,ux,uy,Ex,Ey),2)) + Pfh*(flux1 + flux2);
        rhs2 = -2*d1 + tau*S*(Lfc.*(mP-tM*mq));
        
        flux1 = .5*tM'*diag(nx(:))*(f3(rhoM,rhoP,uM,uP,EM,EP));
        flux2 = sum(Fh2.*f3(rhox,rhoy,ux,uy,Ex,Ey),2);
        d1 = PqG*(sum(Dh.*f3(rhox,rhoy,ux,uy,Ex,Ey),2)) + Pfh*(flux1 + flux2);
        rhs3 = -2*d1 + tau*S*(Lfc.*(EP-tM*Eq));
