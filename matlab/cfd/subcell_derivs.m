...
% for i = 1:N+1
%     plot([rf(i) rf(i+1)],ua(i)*[1 1],'-','linewidth',2)
% end
% return
...
M = tril(ones(N+2,N+1),-1); Mt = -triu(ones(N+2,N+1)); 
% I = eye(N+1); tL = I(:,1); tR = I(:,end);
tL = Vf(1,:)';
tR = Vf(2,:)';
Iq = .5*((M+Mt)*Q + ones(N+2,1)*(tL'+tR'));
...
clf
id = 2;
plot(rf,Iq(:,id),'o--','linewidth',2)
hold on
plot(rf,Vfv(:,id),'x--','linewidth',2)
axis([-1 1 -1 1])
return
