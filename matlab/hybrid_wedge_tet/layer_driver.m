...
% single elem for testing
[VX VY VZ] = wedge_nodes(1); VX = VX(:)'; VY = VY(:)'; VZ = VZ(:)';  
VX = VX + .1*randn(size(VX));
VY = VY + .1*randn(size(VX));
VZ = VZ + .1*randn(size(VX));
EToV = 1:length(VX); EToE = ones(1,5); EToF = ones(1,5);
...
f = 3;
Vfq = wedge_sem_basis(N,rf(fids{f}),sf(fids{f}),tf(fids{f}));
Mf = Vfq'*diag(wsJ(fids{f},e))*Vfq; % surface mass
M = (Vq'*diag(wJ(:,e))*Vq);
LIFTfull = M\Mf;
...
err = (Vq*p-uex(xq,yq,zq,0)).^2;
err = sqrt(sum(wJ(:).*err(:)));
fprintf('Initial err = %f\n',err);
...
flux_p = .5*(p_jump - nu_jump);%.*Fscale(:); %Fscale built into stored LIFTs?
