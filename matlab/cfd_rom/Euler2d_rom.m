...
load Usnap_vortex_s100.mat
% load Usnap_vortex_s100_N3K30.mat

Ntotal = size(Usnap,1)/4;

% start later
istart = 1; 
% istart = find(tsnap > 2.5, 1, 'first');
FinalTime = FinalTime-(istart-1)*interval*dt;
Usnap = Usnap(:,istart:end);

U0 = reshape(Usnap(:,1),Nq*K,4);
ids = 1:Ntotal;
...
Nmodes = 150;
CFL = 2.5; % increase CFL

[Vr,Sr,~] = svd([U(:,1:Nmodes) ones(Ntotal,1)],0);
...
if 1
    for i = 1:2:size(Usnap,2)-1
        ...
        vv = rhou;
%         vv = reshape(VrPr*rhou(:)-rhou(:),Np,K);
...
% rho  = reshape(VrPr*rhoq,Nq,K);
% rhou = reshape(VrPr*(rhoq.*uq),Nq,K);
% rhov = reshape(VrPr*(rhoq.*vq),Nq,K);
% E    = reshape(VrPr*(pq/(gamma-1) + .5*rhoq.*(uq.^2+vq.^2)),Nq,K);
rho = reshape(U0(:,1),Nq,K);
rhou = reshape(U0(:,2),Nq,K);
rhov = reshape(U0(:,3),Nq,K);
E = reshape(U0(:,4),Nq,K);
...
for i = 1:Nsteps
    for INTRK = 1:5
        ...
        q1q = reshape(VrPr*V1(rho(:),rhou(:),rhov(:),E(:)),Nq,K);
        q2q = reshape(VrPr*V2(rho(:),rhou(:),rhov(:),E(:)),Nq,K);
        q3q = reshape(VrPr*V3(rho(:),rhou(:),rhov(:),E(:)),Nq,K);
        q4q = reshape(VrPr*V4(rho(:),rhou(:),rhov(:),E(:)),Nq,K);
        ...
        rhs1 = reshape(VrPr*rhs1(:),Nq,K);
        rhs2 = reshape(VrPr*rhs2(:),Nq,K);
        rhs3 = reshape(VrPr*rhs3(:),Nq,K);
        rhs4 = reshape(VrPr*rhs4(:),Nq,K);
    ...
    if  (mod(i,5)==0 || i==Nsteps)
        clf
        %vv = Vp*Pq*((rhou./rho).^2+(rhov./rho).^2);
        vv = Vp*Pq*(rhou);
        color_line3(xp,yp,vv,vv,'.');
        axis equal
        axis tight
...
% colorbar
% subplot(2,1,2)
U1 = reshape(Usnap(:,end-1),Np*K,4);
rhouex = reshape(U1(:,2),Np,K);
vv = Vp*Pq*(rhouex)-vv;
vv = Vp*Pq*rhouex;
color_line3(xp,yp,vv,vv,'.');
axis equal
...
title(sprintf('Rel err = %g\n',sqrt(sum(sum(wJq.*(rhouex-rhou).^2)))/sqrt(sum(sum(wJq.*rhouex.^2)))))
