...
% h = 2+.5*sin(pi*xq).*sin(pi*yq);
% hu = .5*sin(pi*xq).*sin(pi*yq);
% hv = -.5*sin(pi*xq).*sin(pi*yq);
...
if 1
    ...
    for e = 1:K
        ...
        Bx = diag(nxJ(:,e).*wfq);
        By = diag(nyJ(:,e).*wfq);
        QNx = [Qx-.5*E'*Bx*E  .5*E'*Bx;
            -.5*Bx*E .5*Bx];
        QNy = [Qy-.5*E'*By*E .5*E'*By;
            -.5*By*E .5*By];
...
%         Dy{e} = diag(1./[wq; wfq])*(QNy);
        ...
        if e==1
            BNx = [zeros(size(Qx)),zeros(size(E'));
                zeros(size(E)) Bx];
            
            emodal = sum(Pq,2);
            p1 = Pq*(1+xq(:,e)+yq(:,e));
            (emodal'*MJ*Dxmodal - (nxJ(:,e).*wfq)'*Vf)*p1 % should be zero for P1, need to ensure this is zero for higher modes
...
global g b tau
LFc = sqrt(g*hM) + abs(uM.*nx + vM.*ny); % SWE wavespeed
dQ1 = hP-hM;
dQ2 = hP.*uP-hM.*uM;
dQ3 = hP.*vP-hM.*vM;
Lf1 = tau*LFc.*dQ1.*sJ;
...
% % non-skew form
% fSf1 = fSf1 - .5*(nxJ.*fxS1(hM,uM,vM,hM,uM,vM) + nyJ.*fyS1(hM,uM,vM,hM,uM,vM));
% fSf2 = fSf2 - .5*(nxJ.*fxS2(hM,uM,vM,hM,uM,vM) + nyJ.*fyS2(hM,uM,vM,hM,uM,vM));
% fSf3 = fSf3 - .5*(nxJ.*fxS3(hM,uM,vM,hM,uM,vM) + nyJ.*fyS3(hM,uM,vM,hM,uM,vM));
...
for e = 1:K
    ...
    divF1 = sum(Dxe.*FxS1,2) + sum(Dye.*FyS1,2);
    divF2 = sum(Dxe.*FxS2,2) + sum(Dye.*FyS2,2);
    divF3 = sum(Dxe.*FxS3,2) + sum(Dye.*FyS3,2);
    
    rhs1(:,e) = -(VqPNe*divF1 + VqLqe*fSf1(:,e));
    rhs2(:,e) = -(VqPNe*divF2 + VqLqe*fSf2(:,e));
    rhs3(:,e) = -(VqPNe*divF3 + VqLqe*fSf3(:,e));
end
