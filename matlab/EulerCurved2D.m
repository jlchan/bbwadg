...
if 1
    ...
    [k1,f1] = find(Bfaces==1);
    [k2,f2] = find(Bfaces==2);
    MakeCylinder2D([k1,f1], 4, 0, 0);
    MakeCylinder2D([k2,f2], 1, 0, 0); % double cylinder    
end
...
if 0
    for t = 0
        [rho u v p] = couetteSolution(xp,yp,t);
        m1 = rho.*u;
        m2 = rho.*v;
...
[rho, u, v, p] = couetteSolution(x,y,0);
m1 = rho.*u;
m2 = rho.*v;
E = p/(gamma-1) + .5*rho.*(u.^2 + v.^2);
...
[rhoex, uex, vex, pex] = couetteSolution(x,y,0);
max(abs(rho(:) - rhoex(:)))
...
du(mapB) = 0; dv(mapB) = 0; dp(mapB) = -2*pf(mapB);
...
[rhoBC uBC vBC pBC] = couetteSolution(xf(mapB),yf(mapB),0);
m1BC = rhoBC.*uBC; m2BC = rhoBC.*vBC;
EBC = pBC/(gamma-1) + .5*rhoBC.*(uBC.^2 + vBC.^2);
[FBC GBC] = EulerFluxes(rhoBC,m1BC,m2BC,EBC);
QBC{1} = rhoBC; QBC{2} = m1BC; QBC{3} = m2BC; QBC{4} = EBC;
...
function [rho u v p] = couetteSolution(x,y,t)

global gamma
theta = atan2(y,x);
r = sqrt(x.^2 + y.^2);
utheta = 1/75 * (-r + 16./r);
p = 1 + 1/75^2 * (r.^2/2 - 32*log(r) - 128./(r.^2));
m1 = -sin(theta).*utheta;
m2 = cos(theta).*utheta;
rho = ones(size(x));
E = p/(gamma-1) + .5*rho.*utheta.^2;
