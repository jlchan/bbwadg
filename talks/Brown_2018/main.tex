...
\title[Entropy stable DG]{Entropy stable schemes based on modal discontinuous Galerkin formulations}
\date[9/24/18]{Department of Applied Mathematics, Brown University\\December 7, 2018}
...
\begin{figure}
\centering
\subfloat{\includegraphics[width=.49\textwidth]{figs/gll1Dsbp.pdf}}
\hspace{1.5em}
\subfloat{\raisebox{1em}{\includegraphics[width=.375\textwidth]{figs/B_SBP.png}}}
\end{figure}
\vspace{-.25em}
\begin{itemize}
...
\[
\bm{M}\td{\bm{u}}{t} + \frac{1}{3}\LRp{\bm{Q}\LRp{\bm{u}^{2}} + \diag{\bm{u}}\bm{Q}\bm{u}} + \bm{B}\bm{f}^*= 0,  \qquad \bm{f}^* =  \begin{bmatrix}
\bm{f}_0^*\\
\vdots\\
\bm{f}_N^*
\end{bmatrix}.
\]
\item Semi-discrete stability: multiply by $\bm{u}^T$.  Use that diagonal matrices commute + SBP property to eliminate volume terms
\end{itemize}
\vspace{-.5em}
...
\section{Entropy stable modal formulations and flux differencing}
\frame[noframenumbering]{
\frametitle{Talk outline}
\tableofcontents[currentsection]
}
...
\vspace{.25em}
\item<2-> No restrictions on quadrature (e.g.\ boundary points, ``collocation'')
\vspace{.25em}
\item<4-> Can extend to non-polynomial bases (e.g.\ B-splines, pyramids)
\end{itemize}
...
\begin{figure}
\centering
\includegraphics[width=.75\textwidth]{figs/polymap1.pdf}
\end{figure}
\vspace{-.75em}
\begin{itemize}
\item Assume degree $2N$ volume + surface quadratures $(\bm{x}^q_i,\bm{w}^q_i)$, $(\bm{x}^f_i, \bm{w}^f_i)$, and basis $\phi_1,\ldots,\phi_{N_p}$.  Define interpolation and weight matrices %$\bm{V}_q, \bm{V}_f, \bm{W}, \bm{W}_f$
\begin{align*}
...
\label{eq:DN}
\end{align*}
\item If $\bm{Q}_i + \bm{Q}_i^T = \bm{E}^T\bm{B}_i\bm{E}$, then $\bm{Q}^i_N$ satisfies the SBP property
\begin{gather*}
\boxed{\bm{Q}^i_N + \LRp{\bm{Q}^i_N}^T = \bm{B}^i_N,} \qquad \bm{B}^i_N = \LRs{\begin{array}{cc}
\bm{0} & \\
& \bm{B}_i
\end{array}},\\
\sim \boxed{\int_{D^k} \pd{f}{x_i} g + f\pd{g}{x_i} = \int_{\partial D^k} fg\bm{n}_i.}
\end{gather*}
%\[
%\]
\end{itemize}
...
%\vspace{.25em}
\item $\bm{Q}_N$ approximates ${u}(x) \approx f\pd{g}{x}$ at $\bm{x} = [\bm{x}^q, \bm{x}^f]$ by solving for $\bm{u}$
\[
%f\pd{g}{x} \approx 
...
\vspace{.25em}
\item Equivalent to a skew-symmetric variational problem for $u(\bm{x}) \approx f\pd{g}{x}$.
\[
...
\end{align*}
\item Formulation is energy stable for arbitrary bases and quadratures.
\[
...
\addplot[smooth,color=magenta, dotted, semithick, mark options={solid,fill=markercolor}]
...
\begin{itemize}
\item SBP property lost if quadrature rules are not sufficiently accurate.
\item \note{Skew-symmetric formulation} is entropy stability under relaxed requirements on quadrature accuracy.
\end{itemize}
\[
\bm{M}\td{\hat{\bm{u}}}{t} + \sum_{i=1}^d\LRs{\begin{array}{c}
...
\begin{figure}
\centering
\subfloat[Conforming surface quadrature nodes]{\includegraphics[height=.32\textheight]{figs/aligned.png}}
\hspace{5em}
\subfloat[Non-conforming surface nodes]{\includegraphics[height=.32\textheight]{figs/nonaligned.png}}
\end{figure}
}% only
