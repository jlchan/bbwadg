...
\title[Discretely stable methods]{Discretely entropy stable discontinuous Galerkin methods for nonlinear conservation laws }
\date[July 25, 2018]{WCCM 2018\\July 25, 2018}
...
\frame{
\frametitle{Why are discretizations of nonlinear PDEs so unstable?}
\setcounter{subfigure}{0}
\vspace{-1em}
\begin{figure}
\begin{overprint}
...
\vspace{-.5em}
\begin{itemize}
\item Analogue of energy for nonlinear systems of conservation laws
\[
\pd{\bm{u}}{t} + \pd{\bm{f}(\bm{u})}{x} = 0.  
\]
%\item Examples: Burgers' equation $u^2/2$, shallow water
\item Weak solutions satisfy a continuous entropy inequality: convex \note{entropy} function $S(\bm{u})$ and ``entropy potential'' $\psi(\bm{u})$.  
\begin{align*}
...
\frame{
\frametitle{Examples of entropy functions}

\begin{itemize}
\item Burgers': square entropy $S(\bm{u}) = u^2/2$, entropy variables $\bm{v}(\bm{u}) = u$.
\item Shallow water: entropy is total energy, convex if $h > 0$.
\[
S(\bm{u}) = \frac{1}{2}hu^2 + \frac{1}{2}gh^2 + ghb, \qquad   \bm{v}(\bm{u}) = \begin{bmatrix}
g(h + b) - \frac{u^2}{2} \\
u\end{bmatrix}.
\]
...
%\]
\item Compressible Euler equations: physical entropy $s(\bm{u})$ always increases; {mathematical entropy $S(\bm{u})$} always decreases.  
\begin{gather*}
...
\frame{
\frametitle{Entropy stable schemes in a nutshell}
\begin{columns}
\begin{column}{.65\textwidth}
\begin{figure}
\centering
\vspace{-.5em}
\includegraphics[width=.8\textwidth]{figs/gll1D.png}
\end{figure}
\end{column}
\begin{column}{.3\textwidth}
\[
\bm{D}_{ij} = \LRl{\pd{\ell_j}{x}}_{x=x_i}
\]
\end{column}
\end{columns}
\vspace{1em}
\begin{itemize}
\item<1-> Collocation scheme using Gauss-Legendre-Lobatto quadrature points 
\[
\td{\bm{u}}{t} + \bm{D}\bm{f}(\bm{u}) = 0 \quad  \Longrightarrow \quad \td{\bm{u}_i}{t} + \sum_{j} \bm{D}_{ij} \bm{f}\LRp{\bm{u}_i} = 0.
\]
\item<2-> Flux differencing: recover above form if $\bm{f}_S(\bm{u}_i,\bm{u}_j) = \frac{1}{2}\LRp{\bm{u}_i+\bm{u}_j}$.
\[
...
\]
\item<3-> Semi-discrete entropy equality (add dissipation for inequality)
\[
\bm{M}\td{S(\bm{u})}{t} + \bm{1}^T\bm{B}
\LRp{\bm{v}^T\bm{f}(\bm{u}) - \psi(\bm{u})} = 0.%, \qquad \bm{B} = \begin{bmatrix}
%-1 & &\\
%& \ddots &\\
%& & 1
%\end{bmatrix} 
...
\frame{
\frametitle{Brief overview of entropy stable high order schemes}
\setcounter{subfigure}{0}
\vspace{-.75em}
\begin{figure}
\centering
...
\vspace{-.25em}
\begin{itemize}
\item<1-> SBP finite difference schemes: \note{semi-discrete entropy inequality}.  Includes DG-SEM (Gauss-Legendre-Lobatto collocation) methods.
\vspace{.25em}
%\item Collocation at GLL nodes: underintegration errors.  
\item<2-> Generalized SBP (e.g.\ Gauss collocation): higher accuracy, but requires \note{non-compact coupling conditions} b/w neighboring elements.
\vspace{.25em}
\item<3-> Tetrahedra, wedges, pyramids: how to address over-integration?
\end{itemize}

\uncover<4>{
\vspace{-1em}
\begin{center}
\minibox[frame]{My focus: \note{entropy stability}, \note{compact couplings}, arbitrary \note{quadratures}.}
\end{center}
}
\let\thefootnote\relax\footnotetext{\tiny Fisher and Carpenter (2013), Gassner, Winters, and Kopriva (2016), Chen and Shu (2017), Crean, Hicken, et al.\ (2018). }
}
...
\frame{
\frametitle{Making DG look like FD:  ``decoupled'' SBP operator}
\begin{itemize}
...
\frame{
\frametitle{Accuracy of $\bm{D}_N^i$}

\begin{figure}
\centering
\includegraphics[width=.9\textwidth]{figs/decoupledSBPfig.png}
\end{figure}
}

%\frame{
%\frametitle{Making DG look like FD: quadrature-based matrices }
%
%\begin{itemize}
...
%\]
%\vspace{.1em}
%\item<2-> Key tools: quadrature-based $L^2$ \note{projection} and \note{lifting} matrices
%\[
%\bm{P}_q = \bm{M}^{-1}\bm{V}_q^T\bm{W}, \qquad \bm{L}_f = \bm{M}^{-1}\bm{V}_f^T\bm{W}_f.  
%\]
%\item<3-> $\bm{D}^i_q = \bm{V}_q\bm{D}^i\bm{P}_q$: evaluates $i$th derivative of $L^2$ projection at $\bm{x}^q_i$.
%\[
%\bm{W}\bm{D}^i_q + \LRp{\bm{W}\bm{D}^i_q }^T = \LRp{\bm{V}_f\bm{P}_q}^T\bm{W}_f{\rm diag}\LRp{\bm{n}_i}\bm{V}_f\bm{P}_q, \qquad \text{(SBP property)}.  
%\]
%\end{itemize}
%}
...
%\]
%\item<3-> $\bm{Q}^i_N = \diag\LRp{\begin{bmatrix} \bm{w}^q & \bm{w}_f \end{bmatrix}}
%\bm{D}^i_N$ also satisfies an SBP property
%\[
%\boxed{\bm{Q}^i_N + \LRp{\bm{Q}^i_N}^T = \bm{B}_N} \sim \boxed{\int_{D^k} \pd{f}{x_i} g + f\pd{g}{x_i} = \int_{\partial D^k} fg\bm{n}_i}.
%\]
%%\begin{align*}
...
\bm{V}_q\\
\bm{V}_f
\end{array}} \hat{\bm{u}}, \quad \hat{\bm{u}} = \text{ coefficients},  \qquad \bm{f}^*(u^+,u) = \text{numerical flux} \\
&\td{\hat{\bm{u}}}{t} + \frac{1}{3}
\LRs{\begin{array}{cc}
\bm{P}_q & \bm{L}_f\end{array}}
...
\end{align*}
%\vspace{.01}
\item<5-> Extends beyond split formulations: mass flux for compressible Euler 
\[
...
\item Uses $\tilde{\bm{u}} = \bm{u}\LRp{P_N\bm{v}}$ in terms of \note{$L^2$ projected} entropy variables $P_N \bm{v}$!
...
%&=  \bm{v}_q\bm{W}\td{(\bm{V}_q\hat{\bm{u}})}{t} = \bm{1}^T\bm{W}\td{S(\bm{u}_q)}{\bm{u}_q}\td{\bm{u}_q}{t} = \td{{S}(\bm{u}_q)}{t}.
...
%\frame{
%\frametitle{Recovering (some) finite difference SBP formulations}
%\vspace{-1em}
%\begin{figure}
%\centering
...
%\setcounter{subfigure}{0}
%\subfloat[Generalized SBP]{\includegraphics[height=.325\textheight]{figs/gsbp.png}}
%\hspace{2em}
%\subfloat[Staggered-grid]{\includegraphics[height=.325\textheight]{figs/staggered.png}}
%\hspace{1em}
%%\subfloat[Triangular SBP]{\includegraphics[height=.35\textheight]{figs/chenShuNodes.png}}
%\subfloat[Triangular SBP]{\includegraphics[height=.325\textheight]{figs/creanSBP.png}}
%%\endgroup
%\end{figure}
%\begin{itemize}
%\item Specific choices of basis and quadrature recover existing entropy stable schemes (DG-SEM, Gauss collocation, staggered grid, etc).  
%\vspace{.25em}
%\item Setting $\bm{P}_q = \bm{I}$ recovers non-basis SBP finite differences on simplices.  
%\vspace{.25em}
%\item Generates \note{new compact inter-element coupling terms} (SBP-SATs).  
%\end{itemize}
%
%\let\thefootnote\relax\footnotetext{\tiny Parsani et al.\  (2016). \emph{ES staggered grid disc.\ spectral collocation methods of any order for the comp.\ NS eqns.}}
...
\begin{itemize}
\item Circles are cell averages.  No limiting or artificial viscosity applied.
...
\item Entropy stable wall boundary conditions for GSBP still needed.
\end{itemize}
...
\frame{
\frametitle{2D and 3D isentropic vortex: affine and curved meshes}
\vspace{-1em}
\begin{figure}
\centering
\only<1>{
...
%\hspace{1em}
\subfloat[$\int_{\Omega}U(\bm{u}) \rightarrow 0$ without dissipation]{
\begin{tikzpicture}
\begin{loglogaxis}[
        scaled ticks=false, 
        tick label style={/pgf/number format/fixed},
...
\item $\int_{\Omega} U(\bm{u}) \rightarrow 0$ as ${\rm CFL}\rightarrow 0$ for entropy conservative scheme. 
\end{itemize}
}
...
\begin{itemize}
\item More general class of high order DG methods which satisfy a semi-discrete entropy inequality. 
\vspace{.25em}
\item Challenges: BCs, positivity, strong shocks, computational cost.  
\vspace{.25em}
\item Current work: SWE, Gauss collocation (with DCDR Fernandez, M.\ Carpenter), adaptivity + hybrid meshes, multi-GPU (with L.\ Wilcox).  
\vspace{.25em}
...
\frame[noframenumbering]{
\frametitle{Entropy stable shallow water fluxes}
...
\end{align*}
\item Entropy stable finite volume fluxes (include $b$ for well-balanced)
\begin{align*}
\bm{f}_S\LRp{\bm{u}_L,\bm{u}_R} =
\begin{bmatrix}
\avg{hu}\\
\avg{hu}\avg{u} + \frac{1}{2} g h_L h_R
\end{bmatrix}
\end{align*}
\item For collocation: equivalent to \note{split} form of momentum equation.
\[
\pd{hu}{t} + \frac{1}{2}\LRp{\pd{{hu^2}}{x} + hu \pd{u}{x}}  + gh\pd{(h+b)}{x} =0.
\]
\end{itemize}
...
\subfloat[Degree $N$ GLL, $(N+1)$ points]{\includegraphics[width=.45\textwidth]{figs/sbpGLL.png}}
\hspace{1em}
\subfloat[Degree $N$ GLL, $(N+4)$ points]{\includegraphics[width=.45\textwidth]{figs/sbpGLLNp4.png}}
\endgroup
