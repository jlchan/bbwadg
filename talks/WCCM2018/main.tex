...
\begin{figure}
\centering
\includegraphics[width=.5\textwidth]{figs/gll1D.png}
\end{figure}

\begin{itemize}
\item<1-> Collocate solution at Gauss-Legendre-Lobatto nodes 
\[
\td{\bm{u}}{t} + \bm{D}\bm{f}(\bm{u}) = 0 \quad  \Longrightarrow \quad \td{\bm{u}_i}{t} + \sum_{j} \bm{D}_{ij} \bm{f}\LRp{\bm{u}_i} = 0,
\qquad \bm{D}_{ij} = \LRl{\pd{\ell_j}{x}}_{x=x_i}.
\]
\item<2-> Flux differencing: recover above form if $\bm{f}_S(\bm{u}_i,\bm{u}_j) = \frac{1}{2}\LRp{\bm{u}_i+\bm{u}_j}$.
\[
\td{\bm{u}_i}{t} + \sum_{j} \bm{D}_{ij} \note{\bm{f}_S\LRp{\bm{u}_i,\bm{u}_j}} = 0 \quad \Longrightarrow \quad \td{\bm{u}}{t} + \LRp{\bm{D}\circ\bm{F}_S}\bm{1} = 0.
\]
...
\vspace{.25em}
\item Current work: Gauss collocation (with DCDR Fernandez, M.\ Carpenter), adaptivity + hybrid meshes, multi-GPU (with L.\ Wilcox).  
\vspace{.25em}
