...
\date[February 28, 2017]{SIAM CSE\\February 28, 2017}
\author[Chan, Hewett, Warburton]{Jesse Chan\inst{1}, Russell Hewett\inst{2}, T.\ Warburton\inst{3}}
\institute{\inst{1}Department of Computational and Applied Math, Rice University\\ \inst{2}TOTAL E\&P Research and Technology USA\\ \inst{3}Department of Mathematics, Virginia Tech}
...
\[
\pd{}{t}{}\LRp{\sum_{k} \int_{D^k} \frac{p^2}{c^2} + \LRb{\bm{u}}^2} = -\int_{\partial D^k} \tau_p\jump{p}^2 +  \tau_u\jump{\bm{u}\cdot\bm{n}}^2 \leq 0.
\]
\end{itemize}
}
...
\setcounter{subfigure}{0}
\begin{itemize}
\item Efficient implementation on \textbf{simplicial} meshes: \only<1>{$c^2$}\only<2->{\note{$c^2$}} piecewise constant, non-curved meshes (\only<1>{$J, J^f$}\only<2->{\note{$J, J^f$}} piecewise constant).  
\vspace{.1em}
\begin{align*}
\only<1>{
\int_{D^k}\frac{1}{c^2}\pd{p}{t}{} q &= \int_{D^k} \Div \bm{u} q  + \frac{1}{2}\int_{\partial D^k} \LRp{\jump{\bm{u}}\cdot{\bm{n}} + \tau_p\jump{p}} q\\
...
\vspace{.5em}
\item On-the-fly assembly + inversion or pre-computation and \note{storage}.
\end{itemize}
}
...
\]
\item WADG \textit{a-priori} estimates: standard DG $O(h^{N+1/2})$ convergence of $L^2$ error based on optimal weighted projection estimate:
\begin{align*}
...
\end{align*}
\vspace{.1em}
\item High order Sobolev norm of $J$ - implies that convergence can suffer if mapping is not sufficiently regular.
\end{itemize}
}
...
\frame{
\frametitle{Curvilinear meshes: verification}
...
\vspace{1em}
\begin{center}
Straightforward to combine curved elements and heterogeneous media\\without compromising efficiency.
\end{center}
}
...
\begin{itemize}
\item Weight-adjusted DG (WADG): \textbf{low-storage} methods for heterogeneous media and curvilinear meshes.
\vspace{1em}
\item Future directions:
\vspace{.5em}
\begin{itemize}
\item Reduce complexity of WADG using Bernstein-Bezier basis.  
\vspace{.5em}
\item Singular weights (e.g.\ pyramids)?
\end{itemize}
\end{itemize}
\vspace{1em}
\begin{center}
...
\let\thefootnote\relax\footnotetext{\tiny Chan, et al.\ 2016.  {Weight-adjusted DG methods: wave propagation in heterogeneous media}.}
\let\thefootnote\relax\footnotetext{\tiny Chan, et al.\ 2016.  {Weight-adjusted DG methods: curvilinear meshes}.  }
\let\thefootnote\relax\footnotetext{\tiny Chan 2017.  {Weight-adjusted DG methods: matrix-valued weights and elastic wave propagation in heterogeneous media}.}
