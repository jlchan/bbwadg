...
\frame{
\frametitle{Accurate numerical methods for hyperbolic PDEs}
...
\hspace{1em}%
\subfloat[8th order method]{ %[$N = 7, K = 9$ (non-aligned mesh)]{
...
\only<3->{ \pd{}{t}\int_{x_i}^{x_{i+1}} \bm{u} + \LRu{\bm{f}(\bm{u})}_{x_i}^{x_{i+1}} = \bm{0}.}
\]
\item<4> Introduce numerical flux $\bm{f}_S(\bm{u}_L,\bm{u}_R)$ at cell interfaces% Discretize integrated form of nonlinear conservation law
\[
...
\begin{align*}
\bm{u} = \begin{bmatrix}\rho\\ u\\ E \end{bmatrix}, \qquad
...
\[
\bm{M}\td{\bm{u}}{t} + 2\LRp{\bm{Q}\circ \bm{F}}\bm{1} + \underbrace{\epsilon\bm{K}\bm{u}}_{\text{viscosity}} = 0.
\]
\item<2-> Note the matrix $\bm{Q}$ satisfies $\note{\bm{Q}=-\bm{Q}^T}$ and $\note{\bm{Q}\bm{1} = \bm{0}}$.
\[
...
\vspace{-1em}
\begin{align*}
2\bm{v}^T\LRp{\bm{Q}\circ \bm{F}}\bm{1} &= \sum_{ij} {\bm{Q}_{ij}} \bm{v}_i^T \bm{f}_S\LRp{\bm{u}_i,\bm{u}_j}\\
...
\phi_1 & \ldots & \phi_N\\
\vertbar & & \vertbar
\end{bmatrix}}, \qquad \phi_i^T\phi_j = \delta_{ij}, \qquad \bm{u} \approx \bm{V} \bm{u}_N.
\]
\item (Online) Galerkin projection of the matrix system 
\[
%{\bm{V}^T\bm{M}\bm{V}} 
\Delta x\td{\bm{u}_N}{t} + 2\bm{V}^T \LRp{{\bm{Q}}\circ \bm{F}}\bm{1} = 0.
\]
\item<2-> Reduced system does not preserve conservation of entropy!  
\end{itemize}
}
