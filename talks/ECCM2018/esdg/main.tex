...
\title[Discretely stable methods]{Entropy stable discontinuous Galerkin methods using arbitrary bases and quadratures }
...
%\frame{
%\frametitle{Quadrature-based and ``decoupled'' SBP matrices}
%
%\begin{itemize}
%\item Matrix $\bm{D}^i_q$: evaluates $i$th derivative of $L^2$ projection at points $\bm{x}^q$.
%\[
%\bm{D}^i_q = \bm{V}_q\bm{D}^i\bm{P}_q.  
%\]
%%\begin{overlayarea}{\textwidth}{.265\textheight}
%\item 
%Summation
%%\only<1>{\note{Integration} (summation)} 
%%\only<2>{{Integration} (\note{summation})}
%by parts property involving the $L^2$ projection:
%\[
%%\only<1>{\int_{\hat{D}} \pd{P_N u}{x_i}v + \int_{\hat{D}} u\pd{P_N v}{x_i} = \int_{\partial \hat{D}} \LRp{P_N u}\LRp{P_N v} \hat{n}_i, \quad \forall u,v \in L^2\LRp{\hat{D}}.}
%%\only<2>{\bm{W}\bm{D}^i_q + \LRp{\bm{W}\bm{D}^i_q }^T = \LRp{\bm{V}_f\bm{P}_q}^T\bm{W}_f{\rm diag}\LRp{\bm{n}_i}\bm{V}_f\bm{P}_q.}
%\bm{W}\bm{D}^i_q + \LRp{\bm{W}\bm{D}^i_q }^T = \LRp{\bm{V}_f\bm{P}_q}^T\bm{W}_f{\rm diag}\LRp{\bm{n}_i}\bm{V}_f\bm{P}_q, \quad \text{(unit normal $\bm{n}$)}.
%\]
%%\end{overlayarea}
%\vspace{.1em}
%%\item<2-> Decoupled SBP: improve approx.\ by incorporating boundary points:
%%\begin{align*}
%%\bm{D}^i_N  &= \LRs{
%%\begin{array}{cc}
...
\hspace{2em}
\subfloat[Curved mesh]{\includegraphics[width=.375\textwidth]{figs/mesh2d_curved_converge2.png}}
\caption{Example of an affine and warped 2D mesh (corresponding to $h = 1$).}
}
\only<2>{
...
\vspace{.25em}
\item Challenges: strong shocks, positivity, computational cost.  
\vspace{.25em}
