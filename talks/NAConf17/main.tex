...
\title[BBDG]{Time-domain Bernstein-B\'{e}zier DG methods on simplices}
\date[27/6/17]{27th Biennial Numerical analysis conference\\University of Strathclyde\\June 27, 2017}
\author[Chan, Warburton]{Jesse Chan, T.\ Warburton}
...
\frame{
\frametitle{Outline and acknowledgments}
...
\section{High order nodal DG methods}
...
\vspace{.5em}
\begin{itemize}
\item Continuous PDE (for illustration: constant advection)
\[
...
\begin{column}{.65\textwidth}
\begin{itemize}
\item Matrix-free evaluation of $\mathbf{M}^{-1}\mathbf{A}$.
\vspace{.25em}
\item Local differentiation and lifting \\
matrices $\mathbf{D}_x$ and $\mathbf{L}_f = \mathbf{M}^{-1}\mathbf{M}_f$.  
\vspace{.25em}
\item Assume (for now) piecewise constant coefficients and affine mappings.
\end{itemize}
\end{column}
\begin{column}{.35\textwidth}
\begin{figure}
\centering
\includegraphics[width=\textwidth]{figs/nodal2D.png}
%\includegraphics[width=\textwidth]{figs/nodal3D.png}
\caption{Nodal bases simplify flux computations.}
\end{figure}
\end{column}
\end{columns}
}
...
%%\item Differentiation matrices w.r.t.\ reference coordinates $\bm{\widehat{x}} = (r,s,t)$ 
%%\[
...
Well-suited to GPUs computing (\textbf{significant} speedups over CPUs)!
\end{center}
}
\end{overlayarea}
}
...
\begin{center}
Problem: (tetrahedral) DG becomes \textbf{expensive} at high orders!
\end{center}
\vspace{-.5em}
\begin{columns}
\begin{column}{.55\textwidth}
...
\vspace{.25em}
\item $O(N^{d+1})$ vs $O(N^{2d})$ work per element (differentiation, lifting).
\vspace{.25em}
...
\frame{
\frametitle{High order nodal DG on tetrahedral meshes} 
...
\item No clear tetrahedral equivalent to spectral differentiation, lift matrices.  
...
\frame{
\frametitle{Bernstein-B\'{e}zier bases for finite element methods}
...
\end{figure}
\vspace{.5em}
\item Recent developments: optimal complexity assembly of finite element matrices, sum factorization (reduced complexity quadrature). 
\vspace{.5em}
\item This work: adapt Bernstein-B\'{e}zier for time-domain DG methods.  
\end{itemize}
\let\thefootnote\relax\footnotetext{\tiny Split multi-span NURBS surfaces into B\'{e}zier patches, https://knowledge.autodesk.com} 
\let\thefootnote\relax\footnotetext{\tiny Ainsworth et al.\ 2011. Bernstein-B\'{e}zier finite elements of arbitrary order and optimal assembly procedures.}
\let\thefootnote\relax\footnotetext{\tiny Kirby 2011.  Fast simplicial finite element algorithms using Bernstein polynomials.}
}

\frame{
\frametitle{Bernstein-B\'{e}zier polynomial bases on simplices}
\vspace{-.5em}
\begin{figure}
\centering
...
\frame{
\frametitle{Bernstein-B\'{e}zier derivatives and degree elevation in 1D}
\begin{itemize}
...
\frame{
\frametitle{Bernstein-B\'{e}zier derivative and degree elevation in 3D}
\setcounter{subfigure}{0}
\begin{itemize}
\item Bernstein-B\'{e}zier barycentric differentiation matrices very sparse.% (max 4 nnz/row).
\vspace{.5em}
...
%\item Bernstein-B\'{e}zier lift matrix displays some sparsity - can be improved.  
\end{itemize}
\vspace{-.5em}
\begin{figure}
\centering
...
\hspace{2em}
%\subfloat[Bernstein-B\'{e}zier lift matrix]{\includegraphics[height=.345\textwidth]{figs/spyLIFT_BB.eps}}
...
\frame{
\frametitle{Stencils for Bernstein-B\'{e}zier derivative matrices}

\begin{itemize}
\item Stencil sizes at most $(d+1)$ in $d$ dimensions.
\item Compute derivatives w.r.t.\ barycentric coordinates.  
\item Stencil values are identical for \textbf{all} barycentric derivatives.
\end{itemize}
\begin{figure}
\centering
...
\caption{Bernstein-B\'{e}zier stencils for a single node (in \textcolor{red}{red}) $N=7$.}
\end{figure}
}
...
\setcounter{subfigure}{0}
\vspace{1em}
The Bernstein-B\'{e}zier lift matrix $\mathbf{L}$ admits a factorization of the form
\[
...
\end{overlayarea}
\let\thefootnote\relax\footnotetext{\tiny Chan, Warburton 2016. {GPU-accelerated Bernstein-B\'{e}zier DG methods for wave problems}.}
}

\frame{
\frametitle{Bernstein-B\'{e}zier lift matrix: optimal complexity application}
...
%\frame{
%\frametitle{Bernstein-B\'{e}zier derivative and lift matrices in 3D}
%\setcounter{subfigure}{0}
%\begin{itemize}
%\item Bernstein-B\'{e}zier barycentric differentiation matrices very sparse.% (max 4 nnz/row).
%%\vspace{.5em}
...
%\end{figure}
%}
%\let\thefootnote\relax\footnotetext{\tiny Chan, Warburton 2015. {GPU-accelerated Bernstein-B\'{e}zier DG methods for wave problems}.}
%}

\begin{frame}
\frametitle{Numerical stability of Bernstein-B\'{e}zier DG}
\vspace{.25em}
\begin{itemize}
%\item Bernstein polynomials are monomials; numerically stable?
\item ``Condition number'' of Bernstein differentiation and lift matrices comparable to that of nodal bases.
\[
...
%\caption*{Condition number $\sigma_1/\sigma_r$ of derivative matrix, lift matrix, and lift decomposition for nodal and Bernstein-B\'{e}zier bases.} %for both nodal and Bernstein-B\'{e}zier bases.}
\caption*{Condition numbers of matrices for nodal and Bernstein-B\'{e}zier bases.} %for both nodal and Bernstein-B\'{e}zier bases.}
}
\only<2>{
\subfloat{
\begin{tikzpicture}
...
%\caption*{Growth of (single precision) $L^2$ error over time for nodal and Bernstein-B\'{e}zier bases. } %Both $N$ and the number of elements are taken to be sufficiently large such that the approximation error is at machine (single) precision.}
\caption*{Evolution of $L^2$ error (acoustics) for nodal and Bernstein-B\'{e}zier bases. } %Both $N$ and the number of elements are taken to be sufficiently large such that the approximation error is at machine (single) precision.}
...
%\frame{
%\frametitle{GPU implementation of BBDG}
%\begin{itemize}
%\item Differentiate w.r.t.\ barycentric coordinates, fixed data requirement of $5$ values per thread.  
%\vspace{1em}
%\item Two implementations of lift matrix: treat as a fixed-bandwidth sparse matrix (sparse)
%\end{itemize}
%}

\frame{
\frametitle{GPU runtime comparisons of BBDG and nodal DG }

\vspace{1em}
\begin{overlayarea}{\textwidth}{.95\textheight}
\begin{center}
Bernstein-B\'{e}zier DG achieves $\approx 2\times$ speedup at moderate orders,\\ and up to $\approx 6\times$ speedup at high orders (for acoustics).
\end{center}
%\begin{center}
%Bernstein-B\'{e}zier DG achieves $\approx 2\times$ speedup at moderate orders,\\ and up to $4\times$ speedup at high orders.
%\end{center}
\vspace{-2em}
\begin{figure}
\centering
...
%\caption*{Per-kernel runtimes for nodal and Bernstein-B\'{e}zier bases.  Runtimes are recorded for ten RK4 timesteps on a mesh of 98304 elements.}
%\caption*{Kernel runtimes for Naive nodal, Blocked nodal, and Bernstein-B\'{e}zier DG implementations (50 RHS evaluations, 98304 elements).}
%\caption*{Kernel runtimes for Standard nodal, Blocked nodal, and Bernstein-B\'{e}zier DG implementations (50 RHS evaluations, 98304 elements).}
...
\begin{axis}[
	...
	title={BB speedup (sparse lift) over nodal},
	xlabel={Degree $N$},
	ylabel={Speedup},
	xmin=.5, xmax=9.5,
	ymin=0,ymax=8,%17.5,	
...
%\caption*{Ratio of runtimes of volume/surface kernels and total RHS evaluation using a Bernstein-B\'{e}zier basis instead of nodal polynomials.}
%\caption*{Speedups achieved over nodal DG by using a Bernstein-B\'{e}zier basis.}
}
%\caption*{Bernstein-B\'{e}zier DG achieves $\approx 2\times$ speedup at moderate orders,\\ and up to $4\times$ speedup at high orders.}
\end{figure}
\only<1>{\vspace{-.5em}}
\only<2>{\vspace{-.85em}}
\[
...
\section{Weight-adjusted DG: beyond low order coefficients/geometry}

\frame{
\frametitle{Extensions: high order models of heterogeneous media}
\begin{itemize}
\item Acoustic wave equation in heterogeneous media
\[
\frac{1}{c^2(\bm{x})}\pd{p}{t}{2} - \Delta p = 0.
\]
\item Piecewise constant $c^2(\bm{x})$ efficient, but generates spurious reflections.
\vspace{.5em}
\item Goal: high order $c^2(\bm{x})$, stability, low computational complexity.  
\end{itemize}
...
\frame{
\frametitle{Weighted mass matrices and weight-adjusted DG}
\begin{itemize}
\item Weighted mass matrix: high order accurate and energy stable, but high storage costs, $O(N^6)$ complexity to apply $\bm{M}^{-1}_{w}$.
\[
\td{}{t}\bm{M}_{w} \bm{u} = \text{right hand side}, \qquad w=1/c^2.
\]
\vspace{.125em}
\item \textcolor{red}{Weight-adjusted DG (WADG)}: energy stable, low storage approximation of weighted mass matrix  
\[
\td{}{t}\bm{M}_{w}\bm{u} \approx \td{}{t}\textcolor{red}{{\bm{M}} \LRp{{\bm{M}}_{1/w}}^{-1} {\bm{M}}} \bm{u} = \text{right hand side}.
\]
\vspace{.125em}
\item Bypass inverse of weighted matrix $\LRp{\bm{M}_{w}}^{-1}$ 
\begin{align*}
{\bm{M}} \LRp{{\bm{M}}_{1/w}}^{-1} {\bm{M}} \td{\bm{U}}{t} &= \bm{A}_h \bm{U} \\
\rightarrow \td{\bm{U}}{t} &= \bm{M}^{-1}{\bm{M}}_{1/w}  \underbrace{\bm{M}^{-1}\bm{A}_h \bm{U} }_{\text{RHS for }w = 1}
\end{align*}
\end{itemize}
}
...
%\vspace{-.5em}
\begin{itemize}
\item $L^2$ convergence between optimal $O(h^{N+1})$, provable $O(h^{N+1/2})$.  
\item Extensions to curved elements, matrix weights (elastodynamics).
\end{itemize}

\let\thefootnote\relax\footnotetext{\tiny Chan, Hewett, Warburton.\ 2016.  {Weight-adjusted DG methods: wave propagation in heterogeneous media}.}
\let\thefootnote\relax\footnotetext{\tiny Chan, Hewett, Warburton.\ 2016.  {Weight-adjusted DG methods: curvilinear meshes}.}
\let\thefootnote\relax\footnotetext{\tiny Chan 2017.  Weight-adjusted DG methods: matrix-valued weights and elastic wave prop.\ in heterogeneous media.}
}
...
%\setcounter{subfigure}{0}
%
%WADG is generalizable to matrix weights and elastic wave propagation.
%%\vspace{-.5em}
%\begin{overlayarea}{\textwidth}{.7\textheight}
%\begin{figure}
%\centering
%\subfloat[Acoustic-elastic waves]{\includegraphics[width=.45\textwidth]{figs/acousticElasticPulseP10.png}}
%\hspace{1em}
%\subfloat[Elastic waves]{\includegraphics[width=.45\textwidth]{figs/pplanew0.png}}
%\caption{${\rm tr}(\bm{\sigma})$ for acoustic-elastic and elastic waves in high order media.}
%\end{figure}
%\end{overlayarea}
%
%\let\thefootnote\relax\footnotetext{\tiny Chan 2017.  Weight-adjusted DG methods: matrix-valued weights and elastic wave prop.\ in heterogeneous media.}
...
\frame{
\frametitle{WADG: low-complexity implementations}
\begin{itemize}
\item Low storage, matrix-free application of $\LRp{\bm{M}_{w}^{-1}\bm{M}}^{-1} = \bm{M}^{-1}\bm{M}_{w}$.  
\[
\LRp{\bm{M}}^{-1} {\bm{M}}_{1/w}\text{RHS} = \underbrace{\widehat{\bm{M}}^{-1} \bm{V}_q^T W}_{\bm{P}_q} {\rm diag}\LRp{1/w} \bm{V}_q \LRp{\text{RHS}}.
\]
\item $O(N^4)$ cost in 3D: sum factorization for $\bm{V}_q$, block LDL for $\widehat{\bm{M}}^{-1}$. 
\begin{figure}
\centering
\includegraphics[width=.225\textwidth]{figs/duffytri.png}
%\hspace{.em}
%$\Longrightarrow$
\includegraphics[width=.225\textwidth]{figs/duffycube.png}
\end{figure}
\item Current work: for \textcolor{red}{fixed} approximations of $w(\bm{x})$, \textcolor{red}{optimal} complexity WADG using polynomial multiplication and truncation.
\end{itemize}

\let\thefootnote\relax\footnotetext{\tiny Kirby 2017. Fast inversion of the simplicial Bernstein mass matrix. }
%\let\thefootnote\relax\footnotetext{\tiny Ainsworth, Andriamaro, Davydov 2011. Bernstein-B\'{e}zier FEM of arbitrary order and optimal assembly procedures.}
\let\thefootnote\relax\footnotetext{\tiny Kirby and Thinh 2012. Fast simplicial quadrature-based finite element operators using Bernstein polynomials.}
}
...
\begin{itemize}
\item Optimal complexity RHS evaluation for time-domain DG.
\vspace{1em}
\item Bernstein-B\'{e}zier sparsity: efficiency at high orders on GPUs.
\end{itemize}
\vspace{.5em}
\begin{center}
Thanks to NSF and TOTAL E\&P Research and Technology USA\\ for their support of this work.
\end{center}
...
\let\thefootnote\relax\footnotetext{\tiny Chan, Hewett, Warburton.\ 2016.  {Weight-adjusted DG methods: curvilinear meshes} (arXiv).  }
\let\thefootnote\relax\footnotetext{\tiny Chan, Warburton 2016. {GPU-accelerated Bernstein-B\'{e}zier DG methods for wave problems} (SISC).}
}
...
\frametitle{Performance comparisons of BBDG and nodal DG}

\vspace{.5em}
\begin{overlayarea}{\textwidth}{.075\textheight}
\begin{center}
BBDG: lower FLOPs per second than nodal DG\ldots\\
\uncover<2>{but maintains throughput/bandwidth as $N$ increases!}
\end{center}
\end{overlayarea}
\vspace{.5em}
...
\legend{Volume, Surface (sparse), Surface (optimal)}
\end{axis}
\end{tikzpicture}
}
\caption{Profiled FLOPS/s for nodal and Bernstein-B\'{e}zier DG methods.}
}
\only<2>{
\subfloat{
\begin{tikzpicture}
...
\caption{Profiled bandwidth for nodal and Bernstein-B\'{e}zier DG methods.}
}
\end{figure}
...
\uncover<3>{\item Computational efficiency: ratio of observed/achievable performance.}
%}
\end{itemize}
\vspace{-1em}
\begin{figure}
...
\frame[noframenumbering]{
\frametitle{Efficiency comparisons of BBDG and nodal DG}
%\begin{itemize}
%\item 
\vspace{.25em}
Bernstein-B\'{e}zier DG: standard implementation, sparse matrices.  
\[
...
\]
%\item We compare Bernstein-B\'{e}zier DG with standard and optimized block-partitioned implementations of nodal DG.  
%\end{itemize}
\vspace{-1em}
\begin{figure}
\centering
...
\frame[noframenumbering]{
\frametitle{Runtime-only comparisons: BBDG, SEM-DG on GPUs}
\vspace{-1.5em}
\begin{figure}
\begin{overlayarea}{\textwidth}{.6\textheight}
\centering
...
%\frame[noframenumbering]{
%\frametitle{Bernstein-B\'{e}zier compared to CUBLAS}
%\vspace{-.5em}
%\begin{center}
%Bernstein-B\'{e}zier DG achieves $\approx 4\times$ speedup at low-moderate orders,\\ and $1.5-2\times$ speedup at high orders.
%\end{center}
%%\begin{center}
%%Bernstein-B\'{e}zier DG achieves $\approx 2\times$ speedup at moderate orders,\\ and up to $4\times$ speedup at high orders.
%%\end{center}
