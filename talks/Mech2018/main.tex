...
\date[5/17/18]{Department of Mechanical Engineering, Rice University\\August 29, 2018}
...
\begin{column}{.5\textwidth}
\begin{itemize}
\item<1-> Focus: \note{high accuracy} in computational mechanics on \note{unstructured meshes}.
\vspace{.5em}
\item<1-> Applications in aerodynamics (acoustics, vorticular flows, turbulence, shocks).
\vspace{.5em}
...
\frame{
\frametitle{High order decreases numerical dissipation}% and dispersion}

%\note{Move numerical dissipation here, use advection + Gaussian.  Also add examples of vortex propagation, turbulence, aeroacoustics to support.  }
\begin{figure}
%\only<1>{
%\setcounter{subfigure}{0}
%\centering
...
\section{Stability of DG: linear PDEs vs nonlinear conservation laws} 
...
\section{Summation by parts finite differences}
...
%\vspace{-.75em}
\begin{itemize}
\item Can construct higher order SBP finite difference matrices.
\vspace{.25em}
\item Explicit construction of SBP matrices from an interpolatory \note{polynomial basis} + Gauss-Legendre-Lobatto \note{quadrature}.  
%\vspace{.25em}
...
\frame{
\frametitle{Summary of entropy stable schemes}
%\begin{columns}
%\begin{column}{.65\textwidth}
%\begin{figure}
%\centering
%\vspace{-.5em}
%\includegraphics[width=.75\textwidth]{figs/gll1D.png}
%\end{figure}
%\end{column}
%\begin{column}{.3\textwidth}
%\[
%\bm{D}_{ij} = \LRu{\pd{\ell_j}{x}}_{x=x_i}
%\]
%\end{column}
%\end{columns}
%\vspace{1em}
\begin{itemize}
\item<1-> Traditional SBP scheme (unstable), ignoring boundary conditions: % for either using Gauss-Legendre-Lobatto quadrature points 
\[
\td{\bm{u}}{t} + \bm{D}\bm{f}(\bm{u}) = 0 \quad  \Longrightarrow \quad \td{\bm{u}_i}{t} + \sum_{j} \bm{D}_{ij} \bm{f}\LRp{\bm{u}_i} = 0.
\]
\item<2-> ``Entropy conservative'' finite volume numerical flux $\bm{f}_S(\bm{u}_L, \bm{u}_R)$.  
\vspace{.75em}
\item<3-> Flux differencing: $\bm{f}_S(\bm{u}_i,\bm{u}_j) = \frac{1}{2}\LRp{\bm{u}_i+\bm{u}_j}$ recovers traditional scheme.
\[
...
\]
\item<4-> Semi-discrete entropy \note{equality} using SBP (modify for \note{inequality})
\[
...
\section{High order DG and summation by parts}
\frame[noframenumbering]{
\frametitle{Talk outline}
\tableofcontents[currentsection]
}
...
\begin{figure}
\centering
\includegraphics[width=.75\textwidth]{figs/polymap.pdf}%\note{Add figure of map to/from quadrature to polynomial.}
\end{figure}
\vspace{-.75em}
\begin{itemize}
\item Assume degree $2N$ volume, surface quadratures $(\bm{x}^q_i,\bm{w}^q_i)$, $(\bm{x}^f_i, \bm{w}^f_i)$, and
%, define diagonal weight matrices $\bm{W} = {\rm diag}\LRp{\bm{w}^q}$, $\bm{W}_f = {\rm diag}\LRp{\bm{w}^f}$.
%Define diagonal quadrature weight matrices 
%\[
%\bm{W} = {\rm diag}\LRp{\bm{w}^q}, \qquad \bm{W}_f = {\rm diag}\LRp{\bm{w}^f}.
%\]
%\vspace{.25em}
%\item Assume 
basis $\phi_1,\ldots,\phi_{N_p}$.  Define interpolation matrices $\bm{V}_q, \bm{V}_f$
\[
...
\]
\item Quadrature may not contain boundary points: complicated \note{interface terms} for coupling neighboring elements or imposing BCs. %$\bm{D}^i_q$ leads to generalized SBP methods; complicated \note{interface terms}.  
\end{itemize}
...
%}
%\hspace{.5em}
%\subfloat[With boundary correction]{
%\begin{tikzpicture}
%\begin{axis}[
%    width=.4\textwidth,
%%    xlabel={$x$ coordinate},
...
\]
\item Equivalent to solving a variational problem for $u(\bm{x}) \approx f\pd{g}{x}$ involving the $L^2$ projection $P_N$ onto degree $N$ polynomials
\[
...
%\frame{
%\frametitle{Dissipation: entropy conservation vs entropy stability}
%
%\begin{itemize}
%\item Introduced high order schemes which discretely conserve entropy.
%\item Dissipation of entropy is necessary at shocks.
%\item Artificial viscosity also ensure 
%\end{itemize} 
%
%\let\thefootnote\relax\footnotetext{\tiny Winters, Derigs, Gassner, and Walch (2017). \textit{A uniquely defined entropy stable matrix dissipation operator for high Mach number ideal MHD and compressible Euler simulations.}}
%}
...
\let\thefootnote\relax\footnotetext{\tiny Gassner, Winters, Kopriva (2016). \textit{Split form nodal DG schemes with SBP property for the compressible Euler equations}.}
}
