...
\date[4/25/18]{NASA Langley Research Center\\May 15, 2018}
...
\subfloat[1D matrix ($N=2$, equispaced pts)]{\raisebox{.25em}{\includegraphics[width=.45\textwidth]{figs/sbp_dcdr.png}}}
\hspace{1em}
\subfloat[2D SBP ($N=7$, GLL nodes)]{\includegraphics[width=.4\textwidth]{figs/SEM_stencil_1.pdf}}
...
\hspace{2em}
\subfloat[Gauss collocation and coupling.]{\includegraphics[width=.4\textwidth]{figs/gsbp_coupling.pdf}}
\end{figure}
\vspace{-.25em}
\begin{itemize}
\item<1-> High order methods on hex meshes with \note{discrete entropy inequality.}  
\vspace{.25em}
%\item Collocation at GLL nodes: underintegration errors.  
\item<1-> Gauss collocation: more accurate but expensive coupling conditions.
\vspace{.25em}
\item<2-> Tetrahedra, wedges, pyramids?  Over-integration?  
\end{itemize}


\visible<3>{
\begin{center}
\minibox[frame]{Goal: \note{entropy stable} high order methods using arbitrary \\
basis functions and volume/surface quadrature points.}
\end{center}
}
\let\thefootnote\relax\footnotetext{\tiny Fisher and Carpenter (2013). \textit{High-order ES finite difference schemes for nonlinear conservation laws: Finite domains.} }
...
\begin{itemize}
\item Idea: incorporate \note{boundary values} into approximation of derivative.
\vspace{.5em}
...
\frame{
\frametitle{Unifying (some) SBP formulations}
\vspace{-.75em}
\begin{figure}
\centering
