...
\date[9/4/19]{University of Toronto Institute of Aerospace, Toronto, CN\\December 3, 2019}
...
Goal: improve stability + robustness by preserving an entropy inequality.
\vspace{-.75em}
\begin{columns}
\begin{column}{.5\textwidth}
...
\]
\item<3-> Let $\bm{f}_S(\bm{u}_i,\bm{u}_j) = \frac{1}{2}\LRp{\bm{f}(\bm{u}_i)+\bm{f}(\bm{u}_j)} = {\bm{F}}_{ij}$.  $\bm{Q}\bm{f}(\bm{u})$ is equivalent to
\[
...
\[
\boxed{\bm{M}\td{\bm{u}}{t} + 2\LRp{\bm{Q}\circ\bm{F}}\bm{1} + \bm{B}\big(\underbrace{\bm{f}_S\LRp{\bm{u}^+,\bm{u}}}_{\bm{f}^*}-\bm{f}(\bm{u})\big) = \bm{0}}
\]
\vspace{.1em}
...
\begin{figure}
\centering
\subfloat[Approximate derivatives]{%
\begin{tikzpicture}
\begin{axis}[
    width=.49\textwidth,
    height=.55\textheight,
...
\vspace{.25em}
\begin{itemize}
\item<1-> Use \textit{hybridized} (block) SBP operator, combine with flux differencing.
\begin{align*}
...
\frame{
\frametitle{Entropy stable Gauss collocation schemes (with MH Carpenter + DCDR Fernandez)}
\begin{columns}
\hspace{-.5em}
...
\hspace{1.75em}
\subfloat[Volume + surface + mortar coupling]{\includegraphics[width=.425\textwidth]{figs/mortar_coupling.png}}
\end{figure}
\begin{gather*}
