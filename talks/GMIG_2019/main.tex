...
\title[Entropy stable discretizations]{Discretely entropy stable discretizations for nonlinear conservation laws: high order finite elements and reduced order modeling}
\date[4/30/19]{GMIG 2019 Project Review, Houston, TX\\April 30, 2019}
...
\begin{itemize}
\item<1-> Focus: accurate simulations of waves, vortices, turbulence, shocks on complex geometries.
\vspace{1em}
...
\section{High order stability for linear vs nonlinear PDEs} 
...
\[
\int_{D_k}\LRp{\pd{u}{t}{} + \pd{u}{x}{}}v + \int_{\partial D_k}{{n}_x \LRp{u^*-u}v} = 0, \qquad \forall v \in P^N\LRp{D^k}.
\]
\end{itemize}
}
\only<2-3>{
...
\]
DG mass matrix decouples across elements,\\inter-element and spatial coupling through $\bm{A}$.
...
\frame{
\frametitle{DG is energy stable for linear advection}
...
%\vspace{.5em}
\item<2-> DG with ``penalty'' flux, where $\jump{u} = u^+ - u$ and parameter $\tau \geq 0$.
\begin{align*}
%\sum_k 
...
\begin{align*}
\pd{}{t} \nor{u}_{L^2([-1,1])}^2  \leq -\sum_k \frac{\tau}{2}\int_{\partial D^k}\jump{u}^2 \diff{x} \leq 0.
%\sum_k\int_{D^k} \LRp{\pd{u}{t} + \pd{u}{x}}v + 
%\int_{\partial D^k} \frac{n_x-\tau\LRb{n_x}}{2} \jump{u}v  = 0, \qquad \forall v \in V_h.
\end{align*}
...
\begin{itemize}
\item Energy estimate implies that $\nor{u}^2_{L^2}$ is non-increasing for $\tau \geq 0$.
\vspace{.25em}
...
\frame{
\frametitle{High order methods are unstable for nonlinear PDEs}
\setcounter{subfigure}{0}
\vspace{-1em}
\begin{figure}
\begin{overprint}
...
\vspace{-.25em}
\begin{itemize}
\item High order methods blow up when under-resolved (e.g.\ shocks).
\vspace{.25em}
\item Two sources of instability: low numerical dissipation, and \note{quadrature error} $\Longrightarrow$ loss of the \note{discrete chain rule} in space.  
\end{itemize}
}
...
\]
\item Continuous entropy inequality in terms of a scalar convex \note{entropy} function $S(\bm{u})$ and ``entropy potential'' $\psi(\bm{u})$, 
\begin{align*}
&\int_{-1}^1 \bm{v}^T\LRp{\pd{\bm{u}}{t} + \pd{\bm{f}(\bm{u})}{x}} = 0, \qquad \boxed{\bm{v}(\bm{u}) = \pd{S}{\bm{u}}} \\
...
\frame{
\frametitle{Empirical stabilization techniques are used in practice}

\begin{overlayarea}{\textwidth}{.85\textheight}
\begin{itemize}
\item<1-> Common fix: \note{stabilize by regularizing} (limiters, art.\ viscosity), but \ldots.  
\end{itemize}
\begin{figure}
\centering
\only<1>{
...
\hspace{1em}
\visible<3>{\raisebox{2.5em}{\includegraphics[width=.49\textwidth]{figs/balancing.pdf}}}
}
\end{figure}
\end{overlayarea}
\let\thefootnote\relax\footnotetext{\tiny Figures courtesy of T.\ Warburton, \href{http://cirpwiki.info/wiki/CMS-Flow_Numerical_Methods}{Coastal Inlets Research Program (CIRP)}, ``Man on Wire'' (2008).}
}
...
\section{More general entropy stable DG methods}
\frame[noframenumbering]{
\frametitle{Talk outline}
\tableofcontents[currentsection]
}
...
\vspace{-.7em}
\begin{itemize}
\item Given degree $2N$ volume + surface quadratures $(\bm{x}^q_i,\bm{w}^q_i)$, $(\bm{x}^f_i, \bm{w}^f_i)$, and basis functions $\phi_i(\bm{x})$, define interpolation and weight matrices %$\bm{V}_q, \bm{V}_f, \bm{W}, \bm{W}_f$
\begin{align*}
...
\includegraphics[width=.45\textwidth]{figs/riemannSmall.png}
\end{figure}

\let\thefootnote\relax\footnotetext{\tiny Periodic on larger domain (``natural'' boundary conditions unstable).}
}
...
\subsection{Hybrid meshes and a weak SBP property}
\frame[noframenumbering]{
\frametitle{Talk outline}
\tableofcontents[currentsection,currentsubsection]
}
...
\begin{figure}
\centering
\subfloat[No SBP (tri.\ under-integrated)]{\includegraphics[width=.45\textwidth]{figs/hybrid2D.png}}
\hspace{1em}
\subfloat[No SBP (quad.\ under-integrated)]{\includegraphics[width=.45\textwidth]{figs/hybrid2D_GQ.png}}
\end{figure}
\vspace{.5em}
\begin{itemize}
\item GSBP property lost if surface quadrature insufficiently accurate.
\item Skew formulation entropy stable under ``weak'' GSBP property
\end{itemize}
\begin{gather*}
\bm{1}^T\bm{Q}_i = \bm{1}^T\LRp{\bm{E}^T\bm{B}_i\bm{E} - \bm{Q}_i^T}= \bm{1}^T\bm{B}_i\bm{E}.\\
\Longrightarrow \int_{\Omega} \pd{P_Nu}{\bm{x}_i}=\int_{\partial \Omega} P_N u \bm{n}_i, \qquad \forall \text{ bounded } u(\bm{x}).
\end{gather*}
%\[
...
%\frame{
%\frametitle{A weak generalized SBP property}
%
%\begin{itemize}
%\item Generalized SBP property: differentiation and boundary interpolation matrices $\bm{Q}_i, \bm{E}$ are related through
%\[
%\bm{Q}_i = \bm{E}^T\bm{B}_i\bm{E} - \bm{Q}_i^T.
%\]
%\item Entropy stability holds under a weaker condition (akin to a ``discrete fundamental theorem of calculus'')
%\begin{gather*}
%\bm{1}^T\bm{Q}_i = \bm{1}^T\LRp{\bm{E}^T\bm{B}_i\bm{E} - \bm{Q}_i^T}= \bm{1}^T\bm{B}_i\bm{E}.\\
%\Longrightarrow \int_{\Omega} \pd{P_Nu}{\bm{x}_i}=\int_{\partial \Omega} P_N u \bm{n}_i, \qquad \forall \text{ bounded } u(\bm{x}).
%\end{gather*}
...
\begin{figure}
\centering
\subfloat[Coarse hybrid mesh]{\raisebox{2.75em}{\includegraphics[width=.45\textwidth]{figs/hybrid_mesh.png}}}
\hspace{.25em}
...
\begin{center}
The skew-symmetric formulation guarantees entropy stability for all\\
combinations of Lobatto and Gauss volume and surface quadratures.
\end{center}
}
...
\frame{
\frametitle{Reduced order models (ROMs)}
\vspace{-.5em}
\begin{figure}
\centering
...
\[
\bm{M}\td{\bm{u}}{t} + \LRp{\bm{Q}\circ \bm{F}_S}\bm{1} + \bm{K}\bm{u} = 0, \qquad \bm{Q} = \begin{bmatrix}
0 & 1 & & -1\\
-1 & 0 & 1 &\\
& \ddots & \ddots & 1 \\
1 & & -1  & 0
...
\begin{itemize}
\item (Offline) compute POD basis from component-wise FOM snapshots.
\[
\bm{V}_N = {\begin{bmatrix} \vertbar & & \vertbar \\
\phi_1 & \ldots & \phi_N\\
\vertbar & & \vertbar
\end{bmatrix}}, \qquad \phi_i^T\phi_j =0, \qquad \bm{u} = \bm{V}_N \bm{u}_N.
\]
\item (Online) Galerkin projection of the matrix system 
\[
{\bm{V}_N^T\bm{M}\bm{V}_N} \td{\bm{u}_N}{t} + \bm{V}_N^T \LRp{{\bm{Q}}\circ \bm{F}_S}\bm{1} = 0.
\]
\item (Online) Entropy projection results in an entropy stable ROM
\begin{gather*}
{\tilde{\bm{u}} = \bm{u}\LRp{\bm{V}_N \bm{V}_N^T \bm{v}\LRp{\bm{V}_N \bm{u}_N}}}, \qquad \LRp{\bm{F}_S}_{ij} = \bm{f}_S\LRp{\tilde{\bm{u}}_i,\tilde{\bm{u}}_j}.
\end{gather*}
\end{itemize}
}
...
\[
{\tilde{\bm{u}} = \note{\bm{u}}\LRp{\bm{V}_N \bm{V}_N^T \note{\bm{v}}\LRp{\bm{V}_N \bm{u}_N}}}, \qquad \LRp{{\bm{Q}}\circ \note{\bm{F}_S}}\bm{1} 
\]
\end{center}
\vspace{-.75em}
\uncover<2>{
...
\begin{align*}
&\bm{V}_N^T\bm{g}(\bm{V}_N\bm{u}_N) \approx\\
&\bm{V}_N(\mathcal{I},:)^T\bm{W}\bm{g}(\bm{V}_N(\mathcal{I},:)\bm{u}_N)
\end{align*}
%\vspace{1.5em}
...
\begin{itemize}
\item<1-> (Offline) compress $\bm{Q}$ using orthonormal ``test'' basis $\tilde{\bm{V}}_N$
\begin{gather*}
\tilde{\bm{V}}_N = {\rm orth}\LRp{
\begin{bmatrix} 
\bm{V}_N & \bm{Q}\bm{V}_N & \bm{1}
\end{bmatrix}}, \qquad
\tilde{\bm{V}}_N^T\bm{Q}\tilde{\bm{V}}_N \in \mathbb{R}^{(2N+1) \times (2N+1)}. %\mathbb{R}^{(d+1)N+1 \times (d+1)N+1}.
\end{gather*}
...
\item<2-> (Offline) compute \note{empirical cubature}, projection, and reduced $\tilde{\bm{Q}}$
\begin{gather*}
\bm{W}_{ij} = w_{ii}\delta_{ij},  \qquad \tilde{\bm{M}}_N = \tilde{\bm{V}}_N(\mathcal{I},:)^T\bm{W}\tilde{\bm{V}}_N(\mathcal{I},:)\\
\tilde{\bm{P}}_q = \tilde{\bm{M}}_N^{-1}\tilde{\bm{V}}_N(\mathcal{I},:)^T\bm{W}, \qquad
\tilde{\bm{Q}} = \tilde{\bm{P}_q}^T \LRp{\tilde{\bm{V}}_N^T\bm{Q}\tilde{\bm{V}}_N} \tilde{\bm{P}}_q. 
\end{gather*}
\item<3-> Since $\tilde{\bm{Q}} = -\tilde{\bm{Q}}^T$ and $\tilde{\bm{Q}}\bm{1} = 0$, reduced system is entropy stable.
\[
\underbrace{\bm{V}_N(\mathcal{I},:)^T\bm{W}\bm{V}_N(\mathcal{I},:)}_{\bm{M}_N} \td{\bm{u}_N}{t} + \bm{V}_N(\mathcal{I},:)^T \LRp{\tilde{\bm{Q}}\circ \bm{F}_S}\bm{1} = 0.
\]
\end{itemize}
...
\caption{Full order model with $100\times 100$ grid, viscosity $\epsilon = 1e-3$, $T = 2.5$.  ROM with $75$ modes, $989$ reduced quadrature points, $1.15 \%$ relative $L^2$ error.}
\end{figure}
...
\begin{itemize}
\item Entropy stable formulations significantly improve robustness of high order DG methods and ROMs.
\vspace{.25em}
\item Current work: ROM estimates and accuracy, positivity preservation.  
\vspace{.25em}
