  ...
  */
  ...
  for (int j = 0; j <= p_N; ++j){
    for (int i = 0; i <= p_N-j; ++i){
      ...
      Lm(i,j) = sizeDhat * factorial(Nj)*factorial(Nj)*factorial(d) / denom;
  ...
  MatrixXd Etq((p_N+1)*(p_N+1),NpTri); // tri to quad
  Etq.fill(0.0);
  int csk = 0;
  for (int i = 0; i <= p_N; ++i){
    MatrixXd V1 = Bern1D(p_N,r1D);
    MatrixXd V2 = Bern1D(p_N-i,r1D);
    MatrixXd Ei1D = mldivide(V1,V2);

    for (int r = 0; r < Ei1D.rows();++r){
      for (int c = 0; c < Ei1D.cols();++c){
	int off = i * (p_N+1);
	Etq(off+r,csk+c) = Ei1D(r,c);
      }
    }
    csk += Ei1D.cols();
  }
  ...
  for (int i = 0; i <= p_N; ++i){
	...
	Etw(off+r,csk1+c) = EiTri(r,c);
      }
    }
    csk1 += EiTri.cols();

    MatrixXd tmp = Etq*EiTri;
    for (int r = 0; r < tmp.rows(); ++r){
      for (int c = 0; c < tmp.cols(); ++c){
	int off = i * (p_N+1)*(p_N+1);
	Eth(off + r, csk2 + c) = tmp(r,c);
      }
    }
    csk2 += tmp.cols();
  }
...
void GordonHallSphere(Mesh *mesh){
  ...
  if (p_N > 2){
	...
	//printf("faceBlend size = %d, %d, dx size = %d\n", faceBlend.rows(),faceBlend.cols(),dx.rows());
  ...
  //cout << "KlistCurvedPlusNbrs = [" << KlistCurvedPlusNbrs << "];" << endl;

  //cout << "x = [" << mesh->x.col(e) << "];" << endl;
  //cout << "y = [" << mesh->y.col(e) << "];" << endl;
  //cout << "z = [" << mesh->z.col(e) << "];" << endl;
  //cout << "Fmask = " << endl << Fmask << endl;
...
void time_curved_kernels(Mesh *mesh,int nsteps){
  ...
  for (int step = 0; step < nsteps; ++step){
    ...
    /*
occa::tic("write_vol_quad (nodal WADG skew)");
    kernel_write_vol_quad6(mesh->K, c_KlistAll,
                           c_vgeoq, c_Jq,
                           c_Vq, c_Vrq, c_Vsq, c_Vtq,
                           c_Q, c_Qtmp);
    device.finish();
    elapsedQ = occa::toc("write_vol_quad (nodal WADG skew)", kernel_write_vol_quad6, gflops, bw * sizeof(dfloat));

    occa::tic("volume (nodal WADG skew split)");
    rk_volume_WADG_skew(mesh->K, c_KlistAll,
    ...
    */
    ...
    /*
    occa::tic("surface (nodal WADG skew)");
    rk_surface_WADG_skew_combine(mesh->K, c_KlistAll,
                                 c_fgeo,c_fgeoq,c_VfqFace,c_Pfq,c_Fmask,c_vmapP,
                                 c_Q, c_rhsQ);
    device.finish();
    dfloat elapsedS = occa::toc("surface (nodal WADG skew)",rk_surface_WADG_skew_combine, gflops, bw * sizeof(dfloat));
...
void RK_step_WADG(Mesh *mesh, dfloat rka, dfloat rkb, dfloat fdt){
  ...
  //  kernel_write_vol_quad6(mesh->KCurved, c_KlistCurved,
  //                         c_vgeoq, c_Jq,
  //                         c_Vq, c_Vrq, c_Vsq, c_Vtq,
  //                       c_Q, c_Qtmp);

  //  rk_volume_WADG_skew(mesh->KCurved, c_KlistCurved,
  //		      c_Pq, c_Prq, c_Psq, c_Ptq,
  //		      c_Qtmp, c_rhsQ);
  ...
  /*
  kernel_write_surf_quad(mesh->KCurvedPlusNbrs, c_KlistCurvedPlusNbrs,
    			 c_Fmask, c_VfqFace, c_Vfq,
  			 c_Q, c_Qf);

  rk_surface_WADG_skew(mesh->KCurved, c_KlistCurved,
    		       c_fgeoq, c_Pfq, c_mapPq,
      		       c_Qf, c_rhsQ);
  */
  ...
  //rk_surface_WADG_skew_face(mesh->KCurved, c_KlistCurved,
    //			    c_fgeo,c_fgeoq,c_VfqFace,c_Pfq,c_Fmask,c_vmapP,
    //			    c_Q, c_rhsQ);
  ...
  /*
   kernel_write_vol_quad4(mesh->KCurved, c_KlistCurved,
                         c_vgeoq, c_Jq,
                         c_Vq, c_Vrq, c_Vsq, c_Vtq,
                         c_Q, c_Qtmp);
  rk_volume_WADG_Qtmp(mesh->KCurved, c_KlistCurved,
  		      c_Pq, c_Qtmp, c_rhsQ);
  */
...
void test_RK(Mesh *mesh, int KblkU){
...
#if 0
  printf("planar kernel\n");
  rk_volume_planar(mesh->KPlanar, c_KlistPlanar,
		   c_vgeo, c_Jplanar, c_Dr, c_Ds, c_Dt, c_Qtest, c_rhsQ);

  c_rhsQ.copyTo(rhsQtmp);
  printf("planar_rhs = [\n");
  int e = mesh->KlistPlanar(0);
  for (int i = 0; i < p_Np; ++i){
    printf("%f \n", rhsQtmp[i + p_Nfields*p_Np*e]);
  }
  printf("];\n");

  printf("\n WADGi kernel\n");
  rk_volume_WADG(mesh->KPlanar, c_KlistPlanar,
		 c_vgeoq, c_Jq, c_Vrq, c_Vsq, c_Vtq, c_Pq,
		 c_Qtest, c_rhsQ, c_Qtmp);
  device.finish();
  ...
  cout << "Qtest = [" << endl << Qtest << "];" << endl;

  printf("running write quad pts kernel\n");
  kernel_write_quad_pts(mesh->K, c_KlistAll,
			c_vgeoq, c_Jq,
			c_Vq, c_Vrq, c_Vsq, c_Vtq,
			c_Fmask, c_VfqFace, c_Vfq,
			c_Qtest, c_Qtmp, c_Qf);
  dfloat *Qtmp = (dfloat*) calloc(6 * mesh->K * mesh->Nq,sizeof(dfloat));
  dfloat *Qftmp = (dfloat*) calloc(p_Nfields * mesh->K * mesh->Nfq * p_Nfaces,
				   sizeof(dfloat));
  c_Qtmp.copyTo(Qtmp);
  c_Qf.copyTo(Qftmp);
  printf("Nq = %d\n",mesh->Nq);
  ...
  for (int i = 0; i < mesh->Nfq * p_Nfaces; ++i){
    for (int fld = 0; fld < p_Nfields; ++fld){
      printf("%f ", Qftmp[i + fld*mesh->Nfq*p_Nfaces]);
    }
    printf("\n");
  }

  printf("running skew form volume kernel\n");
  rk_volume_WADG_skew(mesh->K, c_KlistAll,
		      c_Pq, c_Prq, c_Psq, c_Ptq,
		      c_Qtmp, c_rhsQ);

  dfloat *rhsQ = (dfloat*) calloc(p_Nfields * mesh->K * p_Np,sizeof(dfloat));
  c_rhsQ.copyTo(rhsQ);
  printf("vol rhsQ_fld = \n");
  for (int i = 0; i < p_Np; ++i){
    for (int fld = 0; fld < p_Nfields; ++fld){
      printf("%f ", rhsQ[i + fld*p_Np]);
    }
    printf("\n");
  }

  printf("running skew form surface kernel\n");
  rk_surface_WADG_skew(mesh->K, c_KlistAll,
		       c_fgeoq, c_Pfq, c_mapPq,
		       c_Qf, c_rhsQ);
  ...
  c_rhsQ.copyTo(rhsQ);
  int e = 1;
  printf("total rhsQ for elem %d= \n",e);
  for (int i = 0; i < p_Np; ++i){
    for (int fld = 0; fld < p_Nfields; ++fld){
      printf("%f ", rhsQ[i + fld * p_Np + e*p_Np*p_Nfields]);
    }
    printf("\n");
  }
  ...
  //  rk_surface(mesh->K, c_fgeo, c_Fmask, c_vmapP, c_LIFT, c_Q, c_rhsQ);
  //  device.finish();
  //  dfloat elapsedS = occa::toc("surface (nodal)",rk_surface, 0.0,0.0);
