...
\begin{theorem}
The formulation (\ref{eq:ecdg}) is entropy conservative in the sense that
\[
...
It holds but with respect to specifically defined fluxes.  Need to use $v = 1$ on $D^k$ and then symmetry of the global DG operator $D^x_h$.  

\section{Numerical experiments}
\label{sec:num}
\note{We illustrate the entropy conservation and accuracy of the proposed schemes for three equations.  These gradually increase in complexity - Burgers has entropy variables in test space and skew symmetric form, shallow water has only skew symmetric form, and compressible Euler has neither.  }
