...
\section{Entropy stable DG methods on tensor product elements}
...
\section{Decoupled SBP operators for non-conforming meshes}
...
\section{An efficient mortar-based implementation}

\subsection{Three-dimensional considerations}

\note{We need 3 layers of surface nodes in 3D.  Interpolation to isotropic split-side nodes can be done in a tensor-product fashion.  May not matter: interpolation on 2D faces to a refined set of 4 faces is $O(N^4)$ operations, so it doesn't change the overall cost of the code. }
