...
\subsection{\bnote{An alternative construction of SBP operators}} % variational SBP property}

%\subsection{A variational SBP property}

%The main restriction we wish to overcome 

The property (\ref{eq:dsbp}) relates the polynomial exactness of specific quadrature rules to algebraic properties of quadrature-based matrices.  We will relax accuracy conditions on these quadrature rules by utilizing \note{an alternative construction of $\bm{Q}^i_N$ based the skew-symmetric matrix $\bm{Q}^i - \LRp{\bm{Q}^i}^T$:}
%a weaker ``variational'' version of the SBP property variational, which relies on the exactness of volume and surface quadrature rules only for specific polynomials.  
\bnote{\begin{equation}
\tilde{\bm{Q}}^i_{N} = \begin{bmatrix}
\bm{Q}^i - \LRp{\bm{Q}^i}^T &  \frac{1}{2}\bm{E}^T\bm{B}^i\\
-\frac{1}{2}\bm{B}^i\bm{E} & \frac{1}{2} \bm{B}^i
\end{bmatrix}.
\label{eq:skewQN} 
\end{equation}
It is clear to see that the skew-symmetric construction of $\tilde{\bm{Q}}^i_N$ as defined in (\ref{eq:skewQN}) satisfies the SBP property (\ref{eq:dsbp}) by construction.  However, the accuracy of $\tilde{\bm{Q}}^i_N$ as a differentiation operator becomes slightly more difficult to analyze. }
