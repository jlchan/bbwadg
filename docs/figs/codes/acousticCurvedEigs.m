function acousticCurved_weightedDG

global tau;
tau = 1;

% keyboard
lamDG = acousticCurved_weightedDG_driver(3,0,0);
[lamDGr] = sort(real(lamDG));   
[lamDGi] = sort(imag(lamDG));   
[lamWADG MA] = acousticCurved_weightedDG_driver(3,0,1);
[lamWADGr] = sort(real(lamWADG));   
[lamWADGi] = sort(imag(lamWADG));   
plot(lamDG,'o')
hold on;plot(lamWADG,'x')
keyboard
% [lamWADGr] = sort(real(lamWADG)); 
% [lamWADGi] = sort(imag(lamWADG)); 
% rerr = sort(abs(lamWADGr-lamDGr));
% ierr = sort(abs(lamWADGi-lamDGi));
% eigerr1 = sqrt(rerr.^2+ierr.^2);

% max(real(lamWADG))
% return

h1 = figure(1);
plot(sort(imag(lamDG)),'o'); 
hold on; 
plot(sort(imag(lamWADG)),'.','markersize',16);
legend('DG','WADG','Location','Best')
set(gca,'fontsize',15)
% a = 1.1*max(real(lamDG));
% b = 1.1*max(imag(lamDG));
% axis([-a a -b b])
axis([-100 length(lamWADG)+100 -70 70])
xlabel('Eigenvalue index','fontsize',14)
ylabel('Imaginary part','fontsize',14)
grid on
%print(gcf,'-dpng','../eigstau0.png')
% print(gcf,'-dpng','../eigstau0imag.png')
...
lamDG = acousticCurved_weightedDG_driver(3,0,0);
lamWADG = acousticCurved_weightedDG_driver(3,0,1);
[lamDGr] = sort(real(lamDG));   
[lamWADGr] = sort(real(lamWADG)); 
[lamDGi] = sort(imag(lamDG));   
[lamWADGi] = sort(imag(lamWADG)); 
rerr = sort(abs(lamWADGr-lamDGr));
ierr = sort(abs(lamWADGi-lamDGi));
eigerr2 = sqrt(rerr.^2+ierr.^2);

h2 = figure(2);
plot(lamDG,'o'); hold on; plot(lamWADG,'.','markersize',16);
legend('DG','WADG','Location','Best')
set(gca,'fontsize',15)
% axis equal
b = 1.05*max(imag(lamDG));
axis([1.1*min(real(lamDG)) 5 -b b])
grid on
print(gcf,'-dpng','../eigstau1.png')
% figure(2)
% semilogy(eigerr,'o'); 
% hold on

% bigErr = abs(lamWADG-lamDG)>1;
% plot(lamDG(bigErr),'b.','markersize',32)
% plot(lamWADG(bigErr),'r.','markersize',32)
% [err2 href] = acousticCurved_weightedDG_driver(3,1,1);
% [err3 href] = acousticCurved_weightedDG_driver(3,0,-1);
keyboard
...
for nref = 0:3
    nref
    [L2e href] = acousticCurved_weightedDG_driver(N,nref,0);
    L2err(nref+1) = L2e;
    h(nref+1) = href;
    L2errWADG(nref+1) = acousticCurved_weightedDG_driver(N,nref,1);    
%     L2errP1(nref+1) = acousticCurved_weightedDG_driver(N,nref,-1);
end
disp(sprintf('N = %d, L2 err\n',N))    
print_pgf_coordinates(h,L2err)
disp(sprintf('N = %d, L2 WADG err\n',N))
print_pgf_coordinates(h,L2errWADG)
% disp(sprintf('N = %d, L2 linear geom err\n',N))
% print_pgf_coordinates(h,L2errP1)

function [lam MA] = acousticCurved_weightedDG_driver(Nin,nref,useJprojection)
