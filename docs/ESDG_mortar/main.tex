...
\begin{lemma}
\label{lemma:Qhprops}
Suppose the geometric terms $\bm{G}_{ij}$ satisfy the discrete GCL (\ref{eq:dgcl}), the normals are constructed via (\ref{eq:nJ_Gnhat}), and that $\bm{Q}_{i,h}$ is constructed using (\ref{eq:curvedQ}).  Then, %the conditions for Lemma~\ref{lemma:hsbprefprops} hold.  Then, %$\hat{\bm{Q}}_j$ satisfies the properties %a generalized SBP property% \note{finish here}
\[
...
%\[
%\frac{1}{2}\sum_{j=1}^d {\hat{\bm{B}}_{j,h}\diag{\bm{G}_{ij}} +  \diag{\bm{G}_{ij}}\hat{\bm{B}}_{j,h}} = \begin{bmatrix}
%\bm{0} &\\
%& \bm{B}_i \end{bmatrix}.
%\]
%where the last step uses (\ref{eq:nJ_Gnhat}) and that $\hat{\bm{B}}_{j,h}$ and $\diag{\bm{G}_{ij}}$ are diagonal.  
%
...
%\]
%since $\hat{\bm{Q}}_{j,h}\bm{1} = \bm{0}$ by Lemma~\ref{lemma:hsbprefprops}.  Note that, since $\bm{G}_{ij}$ is constructed using polynomial interpolation, $\bm{G}_{ij} = \LRs{\bm{g}_{ij}, \quad \bm{E}\bm{g}_{ij}}^T$, where $\bm{g}_{ij}$ denotes the values of $G_{ij}$ at volume points.  The remaining term can be expanded out
%\[
%\sum_{j=1}^d \hat{\bm{Q}}_{j,h} \bm{G}_{ij} = \sum_{j=1}^d \begin{bmatrix} \hat{\bm{Q}}_j\bm{g}_{ij} - \hat{\bm{g}}_j^T\bm{G}_{ij} + \bm{E}^T\bm{B}\bm{E}\bm{g}_{ij}\\
%\bm{B}\bm{E}\bm{g}_{ij} - \bm{B}\bm{E}\bm{g}_{ij}
%\end{bmatrix} =
%\sum_{j=1}^d \begin{bmatrix} - \hat{\bm{g}}_j^T\bm{G}_{ij} + \bm{E}^T\bm{B}\bm{E}\bm{g}_{ij}\\
%\bm{0}
%\end{bmatrix} 
%\]
%where we have used that $\sum_{j=1}^d\hat{\bm{Q}}_j\bm{g}_{ij} = \bm{0}$, since $G_{ij}$ satisfies the discrete GCL (\ref{eq:dgcl}), $\bm{g}_{ij}$ is a polynomial of degree less than or equal to $N$, and $\hat{\bm{Q}}_j$ exactly differentiates polynomials of degree $N$.  The remaining terms vanish after an application of the SBP property and the GCL (\ref{eq:dgcl}).  
%\qed\end{proof}
...
\subsection{Extensions to $p$ non-conformity, general quadratures, and non-hexahedral elements}
...
\note{Explain why this isn't as much of an issue - Shadpey \cite{shadpey2019energy} use this for simplicial elements.  The interpolation operators are dense, and using the mortar-based approach wouldn't actually decrease the number of operations.}
