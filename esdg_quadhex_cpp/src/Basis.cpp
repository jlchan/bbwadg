...
MatrixXd GradVandermonde1D(int N, VectorXd r){
  int Np = (N+1);
  MatrixXd Vout(r.rows(),Np);
  for (int i = 0; i <= N; ++i){
    Vout.col(i) = GradJacobiP(r,0,0,i);
  }
  return Vout;
}
...
MatrixXd Vandermonde2DQuad(int N, VectorXd r, VectorXd s){
  ...
  for (int i = 0; i <= N; ++i){
    for (int j = 0; j <= N; ++j){
      Vout.col(sk) = JacobiP(r,0,0,i).array()*JacobiP(s,0,0,j).array();
      ++sk;
    }
  }
  return Vout;
