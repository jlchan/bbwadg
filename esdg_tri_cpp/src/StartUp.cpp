...
void InitRefTri(Mesh *mesh, int N){
  ...
  int Nfaces = 3;
  int Nfq = rq1D.size();
  int NfqNfaces = Nfq*Nfaces;  
  VectorXd rf(NfqNfaces); rf << rq1D, -rq1D, -e;
  VectorXd sf(NfqNfaces); sf << -e, rq1D, -rq1D;
  VectorXd wf(NfqNfaces); wf << wq1D, wq1D, wq1D;
  ...
  MatrixXd Vftmp = Vandermonde2D(N,rf,sf);
  MatrixXd Vq = mrdivide(Vqtmp,V);
  MatrixXd Vf = mrdivide(Vftmp,V);

  MatrixXi Fmask(Nfp,Nfaces);
  int f1 = 0, f2 = 0, f3 = 0;
  for (int i = 0; i < Np; ++i){
    if (fabs(s(i)+1.0)<NODETOL){
      Fmask(f1,0) = i;
      ++f1;
    }
  ...
  mesh->N = N;
  mesh->Np = Np;
  mesh->Nfp = Nfp; // nodal face points
  mesh->Nfaces = Nfaces;
  mesh->Nverts = 3; 
  ...
  e.resize(Nfp); e.fill(1.0);
  VectorXd z(Nfp); z.fill(0.0);  
  VectorXd nrJ(Vf.rows()); nrJ << z, e, -e;
  VectorXd nsJ(Vf.rows()); nsJ << -e, e, z;
  mesh->nrJ = nrJ;
  mesh->nsJ = nsJ;  
  
  /*
  // quadrature-based ops
  MatrixXd M = Vq.transpose()*wq.asDiagonal()*Vq;
  MatrixXd VqTW = Vq.transpose()*wq.asDiagonal();
  MatrixXd Pq = mldivide(M,VqTW);
  
  MatrixXd Qr = Pq.transpose()*M*Dr*Pq;
  MatrixXd Qs = Pq.transpose()*M*Ds*Pq;
  MatrixXd E = Vf*Pq;
  */
...
void QuadMesh2d(Mesh *mesh, int Nx, int Ny){
  ...
  // initialize vertices for connectivity
  /*  
      3-----2
      |     |
      |     |
...
void MapTriNodes(Mesh *mesh){
  ...
  // interp to high order nodes (matlab ordering)
  VectorXd r1(3),s1(3);
  r1 << -1.0, 1.0, -1.0;
  s1 << -1.0, -1.0, 1.0;
  ...
  MatrixXd Vdm1 = Vandermonde2D(1,r1,s1);
  MatrixXd VdmN = Vandermonde2D(1,r,s);
...
void GeometricFactors2d(Mesh *mesh){
  ...
  // all quantities stored at nodal points (must interp before using)
  mesh->rxJ = rxJ;
  mesh->sxJ = sxJ;
  mesh->ryJ = ryJ;
  mesh->syJ = syJ;
...
void Normals2d(Mesh *mesh){
  ...
  VectorXd nrJ = mesh->nrJ;
  VectorXd nsJ = mesh->nsJ;
...
void ConnectElems(Mesh *mesh){
  ...
  MatrixXi fvlist = mesh->fvlist;
  
  int Nfaceverts = fvlist.cols();

  // WARNING: if K too large this can fail.
